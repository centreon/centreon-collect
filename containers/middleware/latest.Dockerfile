...
RUN apt-get update && \
    ...
    apt-get update && \
    apt-get install --allow-unauthenticated -y build-essential curl mysql-client=5.6.39-1debian8 mysql-community-server=5.6.39-1debian8 mysql-server=5.6.39-1debian8 netcat php-cli php-curl php-mysql unicode-data
