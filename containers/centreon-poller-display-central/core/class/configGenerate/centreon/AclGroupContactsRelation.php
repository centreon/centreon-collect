...
class AclGroupContactsRelation extends Object
{

    /**
     * @var table
     */
    protected $table = 'acl_group_contacts_relations';
    ...
    public function getList($clauseObject = null)
    {
        ...
        $first = true;
        $clauseQuery = ' WHERE contact_contact_id IN (';
        foreach ($contacts as $contact) {
            if (!$first) {
                $clauseQuery .= ',';
            }
            $clauseQuery .= $contact['contact_id'];
            $first = false;
        }
        $clauseQuery .= ')';
