...
class AclResourcesService extends Object
{

    /**
     * @var table
     */
    protected $table = 'acl_resources_service_relations';
    ...
    public function getList($clauseObject = null)
    {
        ...
        $first = true;
        $clauseQuery = ' WHERE service_service_id IN (';
        foreach ($services as $service) {
            if (!$first) {
                $clauseQuery .= ',';
            }
            $clauseQuery .= $service['service_service_id'];
            $first = false;
        }
        $clauseQuery .= ')';
