...
This means that the second check may start later than the scheduled time point (12:00:10) if the other first checks are too long. 

When a check completes, it is inserted in _waiting_check_queue and his start will be schedule as soon as a slot in queue is available (queue is a set indexed by expected_start) minus old_start + check_period
