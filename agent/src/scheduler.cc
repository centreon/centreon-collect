...
void scheduler::_start_send_timer() {
  _next_send_time_point +=
      std::chrono::seconds(_conf->config().second_export_period());
...
void scheduler::_send_timer_handler(const boost::system::error_code& err) {
  ...
  if (_current_request->resource_metrics_size() > 0) {
...
void scheduler::_init_export_request() {
  _current_request = std::make_shared<export_metric_request>();
  _host_serv_to_scope_metrics.clear();
}
...
scheduler::default_config() {
  std::shared_ptr<com::centreon::agent::EngineToAgent> ret =
      std::make_shared<com::centreon::agent::EngineToAgent>();
  ret->mutable_config()->set_second_check_interval(1);
  ret->mutable_config()->set_second_export_period(1);
...
void scheduler::update(const engine_to_agent_request_ptr& conf) {
  ...
  _active_check = 0;
  size_t nb_check = 0;
  for (const auto& hst : conf->config().hosts()) {
    nb_check += hst.services().size();
  }

  if (conf->config().second_check_interval() <= 0) {
    SPDLOG_LOGGER_ERROR(
        _logger,
        "second_check_interval cannot be null => no configuration update");
    return;
  }

  SPDLOG_LOGGER_INFO(_logger, "schedule {} checks to execute in {}s", nb_check,
                     conf->config().second_check_interval());

  if (nb_check > 0) {
    duration check_interval =
        std::chrono::microseconds(conf->config().second_check_interval() *
                                  1000000) /
        nb_check;

    time_point next = std::chrono::system_clock::now() + check_interval;
    for (const auto& hst : conf->config().hosts()) {
      for (const auto& serv : hst.services()) {
        if (_logger->level() == spdlog::level::trace) {
          SPDLOG_LOGGER_TRACE(
              _logger,
              "check expected to start at {} for host {} service {} command {}",
              next, hst.host(), serv.service_description(),
              serv.command_line());
        } else {
          SPDLOG_LOGGER_TRACE(
              _logger, "check expected to start at {} for host {} service {}",
              next, hst.host(), serv.service_description());
        }
        _check_queue.emplace(_check_builder(
            _io_context, _logger, next, hst.host(), serv.service_description(),
...
void scheduler::_start_check(const check::pointer& check) {
  ...
  if (_logger->level() <= spdlog::level::trace) {
    SPDLOG_LOGGER_TRACE(
        _logger, "start check for host {} service {} command {}",
        check->get_host(), check->get_service(), check->get_command_line());
  } else {
    SPDLOG_LOGGER_DEBUG(_logger, "start check for host {} service {}",
                        check->get_host(), check->get_service());
  }
  check->start_check(
      std::chrono::seconds(_conf->config().second_check_timeout()));
}
...
void scheduler::_check_handler(
    ...
    const std::list<com::centreon::common::perfdata>& perfdata,
    const std::list<std::string>& outputs) {
  SPDLOG_LOGGER_TRACE(_logger, "end check for host {} service {} command {}",
                      check->get_host(), check->get_service(),
                      check->get_command_line());
  ...
  if (_conf->config().use_examplar()) {
    _store_result_in_metrics_and_examplars(check, status, perfdata, outputs);
  } else {
  ...
  if (_alive) {
    ...
    check->add_duration_to_start_expected(
        std::chrono::seconds(_conf->config().second_check_interval()));
 ...
 * @param outputs
 */
void scheduler::_store_result_in_metrics_and_examplars(
    const check::pointer& check,
    unsigned status,
    const std::list<com::centreon::common::perfdata>& perfdata,
    const std::list<std::string>& outputs) {
  auto& scope_metrics =
      _get_scope_metrics(check->get_host(), check->get_service());
...
scheduler::scope_metric_request& scheduler::_get_scope_metrics(
  ...
  auto exist = _host_serv_to_scope_metrics.find({host, service});
  if (exist != _host_serv_to_scope_metrics.end()) {
    return exist->second;
  }
  ::opentelemetry::proto::metrics::v1::ResourceMetrics* new_res =
      _current_request->add_resource_metrics();

  auto* host_attrib = new_res->mutable_resource()->add_attributes();
  host_attrib->set_key("host.name");
  host_attrib->mutable_value()->set_string_value(host);
  ...
  return _host_serv_to_scope_metrics
      .emplace(std::make_pair(host, service), to_insert)
      .first->second;
}
...
void scheduler::_add_metric_to_scope(
  ...
  data_point->set_as_int(perf.value());
  data_point->set_time_unix_nano(now);
