...
void check_exec::_init() {
  try {
    _process = std::make_shared<detail::process>(
        _io_context, spdlog::default_logger(), get_command_line(),
        std::static_pointer_cast<check_exec>(shared_from_this()));
  } catch (const std::exception& e) {
    SPDLOG_ERROR("fail to create process of cmd_line '{}' : {}",
...
void check_exec::start_check(const duration& timeout) {
  ...
  try {
    ...
    SPDLOG_LOGGER_ERROR(_logger, "host {}, serv {} fail to execute {}: {}",
                        get_host(), get_service(), get_command_line(),
                        e.code().message());
    _io_context->post([me = check::shared_from_this(),
  ...
  if (!outputs.empty()) {
    ...
    if (pipe_pos != std::string::npos) {
      perfs = com::centreon::common::perfdata::parse_perfdata(
          0, 0, outputs.begin()->substr(pipe_pos + 1).c_str(),
