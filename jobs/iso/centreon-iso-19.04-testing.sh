...
containerid=`docker create --privileged $ISO_IMAGE /usr/local/bin/container.sh 19.04`

# Copy construction scripts to container.
docker cp `dirname $0`"/../../containers/iso/centos7/addon/19.04" "$containerid:/tmp/addon"
