...
cp `dirname $0`"/../../../packaging/open-tickets/20.10/$PROJECT.spectemplate" input/
...
put_internal_rpms "20.10" "$DISTRIB" "noarch" "open-tickets" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "standard" "20.10" "$DISTRIB" "noarch" "open-tickets" "$PROJECT-$VERSION-$RELEASE"
fi
