...
# Project.
PROJECT=eslint-config-centreon
...
get_internal_source "eslint-config-centreon/$PROJECT-$VERSION-$RELEASE/$PROJECT-$VERSION.tar.gz"
...
if [ "$BRANCH_NAME" == "master" ] ; then
  ...
  if [ "$BUILD_NUMBER" -gt "1" ] ; then
    npm deprecate @centreon/eslint-config-centreon@"0.0.0-$BRANCH_NAME.0 - 0.0.0-$BRANCH_NAME.$(($BUILD_NUMBER-1))" "deprecate previous versions of branch $BRANCH_NAME"
  fi
