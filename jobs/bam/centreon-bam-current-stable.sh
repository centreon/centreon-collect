...
for phpversion in 53 54 ; do
  $SSH_REPO aws s3 cp --acl public-read --recursive "/srv/sources/bam/stable/$PROJECT-$VERSION-$RELEASE/$PROJECT-$VERSION-php$phpversion.tar.gz" "s3://centreon-download/enterprises/centreon-bam/centreon-bam-3.5/centreon-bam-$VERSION/$SRCHASH/$PROJECT-$VERSION.tar.gz"
done
