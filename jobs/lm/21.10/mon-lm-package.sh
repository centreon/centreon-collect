...
get_internal_source "lm/$PROJECT-$VERSION-$RELEASE/$PROJECT-$VERSION-php73.tar.gz"
...
put_internal_rpms "21.10" "$DISTRIB" "noarch" "lm" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "standard" "21.10" "$DISTRIB" "noarch" "lm" "$PROJECT-$VERSION-$RELEASE"
fi
