...
put_internal_rpms "19.10" "el7" "noarch" "map-server" "$PROJECT-$VERSION-$RELEASE" $FILES_TOMCAT7
SSH_REPO='ssh -o StrictHostKeyChecking=no ubuntu@srvi-repo.int.centreon.com'
$SSH_REPO rpm --resign "/srv/yum/internal/19.10/el7/noarch/map-server/$PROJECT-$VERSION-$RELEASE/*.rpm"
$SSH_REPO createrepo "/srv/yum/internal/19.10/el7/noarch/map-server/$PROJECT-$VERSION-$RELEASE"
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "map" "19.10" "el7" "noarch" "map-server" "$PROJECT-$VERSION-$RELEASE"
fi
