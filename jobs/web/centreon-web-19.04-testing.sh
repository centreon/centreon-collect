...
docker cp `dirname $0`/19.04/mon-web-source.container.sh "$containerid:/usr/local/bin/source.sh"
...
cp packaging-centreon-web/rpm/centreon-19.04.spectemplate input/

# Retrieve additional sources.
cp packaging-centreon-web/src/19.04/* input/
...
put_testing_rpms "standard" "19.04" "el7" "noarch" "web" "$PROJECT-$VERSION-$RELEASE" output-centos7/noarch/*.rpm
