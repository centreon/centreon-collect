...
if [ "$BUILD" '=' 'RELEASE' ] ; then
  ...
  copy_internal_rpms_to_testing "standard" "19.10" "el7" "noarch" "web" "$PROJECT-$VERSION-$RELEASE"
  ...
  $SSH_DOC bash -c "'source /srv/env/documentation/bin/activate ; /srv/prod/readthedocs.org/readthedocs/manage.py update_repos centreon -V 19.10 -p'"
  $SSH_DOC bash -c "'source /srv/env/documentation/bin/activate ; /srv/prod/readthedocs.org/readthedocs/manage_fr.py update_repos centreon -V 19.10 -p'"
...
#
  ...
  promote_canary_rpms_to_unstable "standard" "19.10" "el7" "noarch" "web" "$PROJECT-$VERSION-$RELEASE"
  TARGETVERSION='19.10'
fi
