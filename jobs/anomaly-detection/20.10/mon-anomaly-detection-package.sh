...
put_internal_rpms "20.10" "$DISTRIB" "noarch" "anomaly-detection" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "standard" "20.10" "$DISTRIB" "noarch" "anomaly-detection" "$PROJECT-$VERSION-$RELEASE"
fi
