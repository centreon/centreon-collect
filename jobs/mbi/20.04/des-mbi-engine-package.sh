...
put_internal_rpms "20.04" "$DISTRIB" "noarch" "mbi-engine" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "mbi" "20.04" "el7" "noarch" "mbi-engine" "$PROJECT-$VERSION-$RELEASE"
fi
