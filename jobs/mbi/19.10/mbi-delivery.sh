...
#
  ...
  for distrib in centos7 ; do
    ...
    docker tag "$REGISTRY/des-mbi-server-$VERSION-$RELEASE:$distrib" "$REGISTRY/des-mbi-server-19.10:$distrib"
    docker push "$REGISTRY/des-mbi-server-19.10:$distrib"

    # -web- image.
    docker pull "$REGISTRY/des-mbi-web-$VERSION-$RELEASE:$distrib"
    docker tag "$REGISTRY/des-mbi-web-$VERSION-$RELEASE:$distrib" "$REGISTRY/des-mbi-web-19.10:$distrib"
    docker push "$REGISTRY/des-mbi-web-19.10:$distrib"
  done
