...
void parser::parse_perfdata(uint32_t host_id,
  ...
  log_v2::perfdata()->debug(
      "unified_sql: parsing service {} perfdata string '{}'", id(), buf);
  ...
  while (*tmp) {
    ...
    if (std::isnan(p.value())) {
      ...
      log_v2::perfdata()->error(
          "unified_sql: invalid perfdata format in service {}: no numeric "
          "value "
          "after equal sign "
          "'{}'",
          id(), fmt::string_view(s, (tmp - s) + i));
    ...
    // Log new perfdata.
    log_v2::perfdata()->debug(
        "unified_sql: got new perfdata (name={}, value={}, unit={}, "
        "warning={}, "
        "critical={}, min={}, max={})",
