...
void stream::_unified_sql_process_pb_service_status(
  ...
  auto s{static_cast<const neb::pb_service*>(d.get())};
  auto ss = s->obj();
  ...
  auto add_metric_in_cache = [this](uint64_t index_id, uint64_t host_id,
                                    uint64_t service_id, Service const& ss,
                                    bool index_locked, bool special,
...
void stream::_insert_perfdatas() {
  ...
  if (it != pd_queue.end()) {
    ...
    //_update_status("");
    log_v2::sql()->info("unified_sql: {} perfdata inserted in data_bin", count);
  }
}
