...
TEST(UnifiedSqlPerfdata, Assign) {
  ...
  p1.unit("bar");
  p1.value(52189.912);
  p1.value_type(unified_sql::perfdata::counter);
  p1.warning(4548.0);
  p1.warning_low(42.42);
  p1.warning_mode(true);

  // Second object.
  unified_sql::perfdata p2;
  ...
  p2.value_type(unified_sql::perfdata::absolute);
  ...
  p1.unit("qux");
  p1.value(3485.9);
  p1.value_type(unified_sql::perfdata::derive);
  p1.warning(3612.0);
  ...
  ASSERT_FALSE(p1.value_type() != unified_sql::perfdata::derive);
  ...
  ASSERT_FALSE(p2.value_type() != unified_sql::perfdata::counter);
...
TEST(UnifiedSqlPerfdata, DefaultCtor) {
  ...
  ASSERT_FALSE(p.value_type() != unified_sql::perfdata::gauge);
...
// Given a unified_sql::parser object
...
TEST_F(UnifiedSqlParserParsePerfdata, Simple1) {
  // Parse perfdata.
  std::list<unified_sql::perfdata> lst;
  ...
  std::list<unified_sql::perfdata>::const_iterator it(lst.begin());
  unified_sql::perfdata expected;
  expected.name("time");
  expected.value_type(unified_sql::perfdata::gauge);
...
TEST_F(UnifiedSqlParserParsePerfdata, Simple2) {
  // Parse perfdata.
  std::list<unified_sql::perfdata> list;
  unified_sql::parser p;
  p.parse_perfdata(0, 0, "'ABCD12E'=18.00%;15:;10:;0;100", list);

  // Assertions.
  ASSERT_EQ(list.size(), 1u);
  std::list<unified_sql::perfdata>::const_iterator it(list.begin());
  unified_sql::perfdata expected;
...
TEST_F(UnifiedSqlParserParsePerfdata, Complex1) {
  ...
  // #2.
  expected = unified_sql::perfdata();
  expected.name("metric");
  expected.value_type(unified_sql::perfdata::derive);
  ...
  expected = unified_sql::perfdata();
  expected.name("foo");
  expected.value_type(unified_sql::perfdata::absolute);
  ...
  expected = unified_sql::perfdata();
  expected.name("bar");
  expected.value_type(unified_sql::perfdata::counter);
  expected.value(1234.0);
  expected.warning(10.0);
