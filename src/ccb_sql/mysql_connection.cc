...
void mysql_connection::_statement_res(mysql_task* t) {
  ...
  if (bb && mysql_stmt_bind_param(stmt, bb)) {
    log_v2::sql()->error(
        ...
        mysql_stmt_error(stmt));
    msg_fmt e(mysql_stmt_error(stmt));
    task->promise->set_exception(std::make_exception_ptr<msg_fmt>(e));
  } else {
    int attempts(0);
    while (true) {
...
void mysql_connection::_statement_int(mysql_task* t) {
  ...
  log_v2::sql()->trace("statement {} query: {}",
                       task->statement_id,
                       _stmt_query[task->statement_id]);
  MYSQL_STMT* stmt(_stmt[task->statement_id]);
  if (!stmt) {
...
bool mysql_connection::is_finished() const { return _finished; }
...
void mysql_connection::_run() {
  ...
  if (!_conn)
    ...
    while (!mysql_real_connect(_conn,
                               _host.c_str(),
                               _user.c_str(),
                               _pwd.c_str(),
                               _name.c_str(),
...
int mysql_connection::get_tasks_count() const { return _tasks_count; }
