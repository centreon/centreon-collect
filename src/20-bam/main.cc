...
void register_bam_event(io::events& e, bam::data_element de, const char* name) {
  e.register_event(
      io::events::bam, de, io::event_info(name, &T::operations, T::entries));
}
...
void broker_module_init(void const* arg) {
  ...
  if (!instances++) {
    // BAM module.
    logging::info(logging::high) << "BAM: module for Centreon Broker "
                                 << CENTREON_BROKER_VERSION;

    // Register storage layer.
    io::protocols::instance().reg(
        bam_module, std::make_shared<bam::factory>(), 1, 7);
    ...
    if (bam_category != io::events::bam) {
      ...
      --instances;
      throw msg_fmt(
          "bam: category {} is already registered whereas it should be "
    ...
    // Register bam events.
    {
      ...
      register_bam_event<bam::ba_duration_event>(
          e, bam::de_ba_duration_event, "ba_duration_event");
      register_bam_event<bam::dimension_ba_event>(
          e, bam::de_dimension_ba_event, "dimension_ba_event");
      ...
      register_bam_event<bam::dimension_bv_event>(
          e, bam::de_dimension_bv_event, "dimension_bv_event");
      ...
      register_bam_event<bam::inherited_downtime>(
          e, bam::de_inherited_downtime, "inherited_downtime");
    }
  }
  return;
}
