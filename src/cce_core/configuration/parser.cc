...
void parser::_add_template(object_ptr obj) {
  ...
  auto& file_info = _get_file_info(obj.get());
  if (name.empty())
    throw engine_error(
        "Parsing of {} failed in file {} on line {}: Property 'name' is "
        "missing",
        obj->type_name(),
        file_info.path(),
        file_info.line());
  ...
  if (tmpl.find(name) != tmpl.end())
    throw engine_error("Parsing of {} failed in file {} on line {}: {} already exists",
                obj->type_name(),
                file_info.path(),
                file_info.line(),
                name);
...
void parser::_get_hosts_by_hostgroups(hostgroup const& hostgroups,
  ...
  _get_objects_by_list_name(
      hostgroups.members(), _map_objects[object::host], hosts);
}
...
void parser::_parse_global_configuration(std::string const& path) {
  ...
  if (!stream.is_open())
    throw engine_error("Parsing of global configuration failed: Can't open file '{}'",
                path);
...
void parser::_parse_object_definitions(std::string const& path) {
  ...
  std::ifstream stream(path.c_str(), std::ios::binary);
  if (!stream.is_open())
    throw engine_error("Parsing of object definition failed: Can't open file '{}'",
                path);
...
void parser::_parse_resource_file(std::string const& path) {
  logger(logging::log_info_message, logging::most) << "Reading resource file '"
                                                   << path << "'";

  std::ifstream stream(path.c_str(), std::ios::binary);
  if (!stream.is_open())
    throw engine_error("Parsing of resource file failed: can't open file '{}'", path);
  ...
  for (unsigned int i(0); i < sizeof(_lst_objects) / sizeof(_lst_objects[0]);
    ...
    for (list_object::iterator it(_lst_objects[i].begin()),
      ...
      try {
        ...
        auto& file_info = _get_file_info(it->get());
        // throw engine_error("Configuration parsing failed {}: {}",
        // _get_file_info(it->get()), e.what());
        throw engine_error("Configuration parsing failed in file {} on line {}: {}",
                    file_info.path(),
                    file_info.line(),
                    e.what());
      }
  ...
  for (unsigned int i(0); i < sizeof(_map_objects) / sizeof(_map_objects[0]);
    ...
    for (map_object::iterator it(_map_objects[i].begin()),
      ...
      try {
        ...
        auto& file_info = _get_file_info(it->second.get());
        throw engine_error("Configuration parsing failed in file {} on line {}: {}",
                    file_info.path(),
                    file_info.line(),
                    e.what());
...
void parser::_store_into_map(object_ptr obj) {
  ...
  if (it != _map_objects[obj->type()].end())
    throw engine_error("Parsing of {} failed in file {} on line {}: {} alreadyexists",
                obj->type_name(),
                file_info.path(),
                file_info.line(),
                obj->name());
