...
std::string query::generate_metric(storage::metric const& me) {
  if (_type != metric)
    throw msg_fmt(
        "graphite: attempt to generate metric with a query of the bad type");
  _naming_scheme_index = 0;
  std::ostringstream iss;
  std::ostringstream tmp;
  try {
    for (std::vector<
             void (query::*)(io::data const&, std::ostream&)>::const_iterator
...
std::string query::generate_status(storage::status const& st) {
  if (_type != status)
    throw msg_fmt(
        "graphite: attempt to generate status with a query of the bad type");
  _naming_scheme_index = 0;
  std::ostringstream iss;
  std::ostringstream tmp;
  try {
...
void query::_compile_naming_scheme(std::string const& naming_scheme,
  ...
  while ((found_macro = naming_scheme.find_first_of('$', found_macro)) !=
    ...
    if (macro == "$METRICID$") {
      ...
      logging::config(logging::high) << "graphite: unknown macro '" << macro
                                     << "': ignoring it";
