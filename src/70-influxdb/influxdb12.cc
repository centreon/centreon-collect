...
void influxdb12::commit() {
  ...
  do {
    ...
    if (err)
      throw msg_fmt("influxdb: couldn't receive InfluxDB answer with address '{}' and port '{}': {}", _socket.remote_endpoint().address().to_string(), _socket.remote_endpoint().port(), err.message());
...
void influxdb12::_connect_socket() {
  ...
  try {
    ...
    if (err) {
      throw msg_fmt("influxdb: couldn't connect to InfluxDB with address '{}' and port '{}': {}", _host, _port, err.message());
    }
  } catch (std::system_error const& se) {
    throw msg_fmt("influxdb: couldn't connect to InfluxDB with address '{}' and port '{}': {}", _host, _port, se.what());
  }
}
...
bool influxdb12::_check_answer_string(std::string const& ans) {
  ...
  if ((split[0] == "HTTP/1.0") && (split[1] == "204") && (split[2] == "No") &&
    ...
    throw msg_fmt("influxdb: got an error from '{}' and port '{}': '{}'", _socket.remote_endpoint().address().to_string(), _socket.remote_endpoint().port(), ans);
}
