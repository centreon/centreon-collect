...
void stream::_process_host_state(std::shared_ptr<io::data> const& e) {
  ...
  logging::info(logging::medium) << "SQL: processing host state event (host: "
                                 << s.host_id << ", state: " << s.current_state
  ...
  if (_with_state_events) {
    ...
    _mysql.run_statement(_host_state_insupdate,
                         oss.str(),
                         true,
...
void stream::_process_issue_parent(std::shared_ptr<io::data> const& e) {
  ...
  //        throw msg_fmt("child issue does not exist");
  //
  ...
  //      throw msg_fmt("SQL: could not fetch child issue ID (host: {}, service:
  // {}, start: {}): {}", ip.child_host_id, ip.child_service_id,
  ...
  //      mysql_result res(promise.get_future().get());
  //      if (!_mysql.fetch_row(res))
  //        throw msg_fmt("parent issue does not exist");
  //
  ...
  //          << parent_id;
  //    } catch (std::exception const& e) {
  //      throw msg_fmt("SQL: could not fetch parent issue ID (host: {},
  // service: {}, start: {}): {}", ip.parent_host_id, ip.parent_service_id,
  ...
  //    }
  //  } catch (std::exception const& e) {
  //    throw msg_fmt("SQL: issue parent update query failed: {}", e.what());
  //  }
}
