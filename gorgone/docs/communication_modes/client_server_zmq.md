...
Gorgone can communicate with other Gorgone instances of 3 ways :
* push_zmq (default method)
* [pull_zmq](./poller_pull_configuration.md)
* [pull_wss](./poller_pullwss_configuration.md)

For push_zmq and pull_zmq, the communication is done using the ZeroMQ protocol. pull_wss use an HTTP websocket.

It is possible to make a [rebound configuration](./rebound_configuration.md), where a gorgone instance is used as a proxy to communicate with other gorgone instances.


When using push_zmq, all communications are encrypted using symmetric-key encryption based on public/private keys from both client and server.
