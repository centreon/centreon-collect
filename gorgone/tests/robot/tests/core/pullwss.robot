...
check one poller can connect to a central and gorgone poller stop first
    ...
    # let's change the token : poller should not be able to connect anymore.
    ...
    Execute Sql String    UPDATE nagios_server SET gorgone_auth_token = 'wrong_secret_token' WHERE id = 2
    ...
    # now let's set empty token on both side : connexion should still be impossible, as we don't want unauthenticated poller.
    Execute Sql String    UPDATE nagios_server SET gorgone_auth_token = '' WHERE id = 2
    ${api_resp}=    POST    http://127.0.0.1:8085/api/centreon/nodes/sync    data={}
    ${result}    Run    sed -i -e 's/secret_token//g' /etc/centreon-gorgone/${poller_process}/config.d/pullwss_poller_config.yaml
...
use default token from yaml conf
    ...
    # A     null null   Nok
    # A     null  B     Nok
    Examples:    db_token    poller_token    connect   --
...
Start central With Token In Yaml
    [Documentation]    todo
    TRY
        ${process}=    Is Process Running    ${central_process}
    EXCEPT
        Log To Console    "expect error."
        ...
        ${result}    Run    sed -i -e 's/# token: "secret_token"/token: "secret_token" #/g' /etc/centreon-gorgone/${central_process}/config.d/pullwss_central_config.yaml
        Execute Sql String    UPDATE nagios_server SET gorgone_auth_token = 'secret_token_db' WHERE id = 2
