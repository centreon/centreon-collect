...
bool tcp_connection::wait_for_all_events_written(unsigned ms_timeout) {
  _logger->trace("wait_for_all_events_written _writing={}", _writing);
  std::mutex dummy;
...
void tcp_connection::handle_write(const boost::system::error_code& ec) {
  if (ec) {
    _logger->error("Error while writing on tcp socket: {}", ec.message());
    std::lock_guard<std::mutex> lck(_error_m);
    _current_error = ec;
    _writing = false;
    _closed = true;
...
void tcp_connection::handle_read(const boost::system::error_code& ec,
  ...
  if (ec) {
    _logger->error("Error while reading on socket: {}", ec.message());
