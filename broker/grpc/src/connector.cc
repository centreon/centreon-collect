...
class client_stream : public stream_base_class {
 ...
 public:
  client_stream(const io::endpoint* parent,
...
client_stream::client_stream(const io::endpoint* parent,
                             ...
                             const std::shared_ptr<asio::io_context> io_context,
                             const std::shared_ptr<spdlog::logger>& logger)
    : stream_base_class(parent, conf, "client", io_context, logger) {
...
std::shared_ptr<io::stream> connector::create_stream() {
  std::shared_ptr<client_stream> new_stream = std::make_shared<client_stream>(
      this, std::static_pointer_cast<grpc_config>(get_conf()), get_io_context(),
      get_logger());
