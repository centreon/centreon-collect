...
#ifndef CCB_BBDO_BASIC_STREAM_HH
#define CCB_BBDO_BASIC_STREAM_HH
 ...
 */
class basic_stream : public io::stream {
  class buffer {
    uint32_t _event_id;
    uint32_t _source_id;
    uint32_t _dest_id;
 ...
 public:
  ...
  const bbdo_version& get_bbdo_version() const;
  int timeout() const { return _timeout; }
  uint64_t poller_id() const { return _poller_id; }
  void set_poller_id(uint64_t id) { _poller_id = id; }
  const std::string& poller_name() const { return _poller_name; }
  void set_poller_name(const std::string& name) { _poller_name = name; }
  const std::string& broker_name() const { return _broker_name; }
  void set_broker_name(const std::string& name) { _broker_name = name; }
  common::PeerType peer_type() const { return _peer_type; }
  void set_peer_type(common::PeerType type) { _peer_type = type; }
};
}  // namespace com::centreon::broker::bbdo

#endif  // !CCB_BBDO_BASIC_STREAM_HH
