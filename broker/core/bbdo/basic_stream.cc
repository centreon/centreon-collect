...
void basic_stream::_handle_bbdo_event(const std::shared_ptr<io::data>& d) {
  switch (d->type()) {
    ...
    default:
      /* A BBDO event this stream has no use for. It reaches here mostly from
       * the retention pipeline, which builds a bare basic_stream
       * (persistent_file.cc): a control message stored in a muxer queue and
       * read back at the next start has no peer to be handled for, and is
       * dropped. */
      SPDLOG_LOGGER_INFO(_logger,
                         "BBDO: event 0x{:x} is not handled by this stream, "
                         "ignoring it",
                         d->type());
      break;
  }
