...
void modules::apply(const std::list<std::string>& module_list,
  ...
  for (const std::string& m : module_list) {
    log_v2::instance()
        .get(log_v2::CONFIG)
        ->info("module applier: loading module '{}'", m);
    if (!load_file(fmt::format("{}/{}", module_dir, m), arg))
      log_v2::instance()
          .get(log_v2::CONFIG)
          ->error("module applier: impossible to load module '{}'", m);
  }
}
...
bool modules::_check_module(const std::string& name, void* h) noexcept {
  ...
  log_v2::instance()
      .get(log_v2::CORE)
      ->debug("modules: checking initialization routine (symbol '{}') in '{}'",
  ...
  log_v2::instance()
      .get(log_v2::CORE)
      ->debug("modules: checking module version (symbol '{}') in '{}'",
  ...
  if (!version) {
    log_v2::instance()
        .get(log_v2::CORE)
        ->error(
            "modules: could not find version in '{}' (not a Centreon Broker "
            "module "
            "?): {}",
            name, dlerror());
    return false;
  ...
  if (::strncmp(CENTREON_BROKER_VERSION, *version,
    ...
    log_v2::instance()
        .get(log_v2::CORE)
        ->error("modules: version mismatch in '{}': expected '{}', found '{}'",
                name, CENTREON_BROKER_VERSION, *version);
    return false;
  }
  return true;
}
...
bool modules::load_file(const std::string& filename, const void* arg) {
  ...
  if (found == _handles.end()) {
    log_v2::instance()
        .get(log_v2::CORE)
        ->info("modules: attempt to load module '{}'", filename);
    void* h = dlopen(filename.c_str(), RTLD_LAZY | RTLD_GLOBAL);
    if (!h)
      log_v2::instance()
          .get(log_v2::CONFIG)
          ->error("modules: could not load module '{}': {}", filename,
                  dlerror());
        ...
        for (auto p = pts; *p; ++p) {
          ...
          if (found == _handles.end())
            if (!load_file(fmt::format("{}/{}", path, *p), arg))
              log_v2::instance()
                  .get(log_v2::CONFIG)
                  ->error("modules: impossible to load parent module '{}'", *p);
        }
      }
    } else {
      if (h)
    ...
    log_v2::instance()
        .get(log_v2::CORE)
        ->info("modules: attempt to load '{}' which is already loaded",
               filename);
