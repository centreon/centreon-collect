...
//
//   gl_acknowledgements[std::make_pair(ack_obj.host_id(),
//   ack_obj.service_id())] =
//       ack;
//
//   // Send event.
...
//         (h->has_been_checked() ? h->get_current_state() : 4);  // Pending
//         state.
...
//       common::check_string_utf8(h->get_display_name());
...
//     h->get_notify_on(engine::notifier::downtime); my_host->notify_on_flapping
//     =
