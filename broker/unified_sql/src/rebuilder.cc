    ...
    try {
      ...
      std::string query{
          fmt::format("SELECT m.metric_id, m.metric_name, m.data_source_type, "
                      "i.rrd_retention, s.check_interval FROM metrics m LEFT "
                      "JOIN index_data "
      ...
      try {
        ...
        if (db_retention_day) {
          ...
          if (localtime_r(&now, &tmv)) {
            /* Let's compute the beginning and the end of the first day where
             * the rebuild starts. */
            ...
            while (db_retention_day >= 0) {
              ...
              db_retention_day--;
              std::promise<database::mysql_result> promise_bin;
              std::future<database::mysql_result> future_bin =
                  promise_bin.get_future();
              std::string query{fmt::format(
                  "SELECT id_metric,ctime,value FROM data_bin WHERE "
                  "ctime>={} AND "
