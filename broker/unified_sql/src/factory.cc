...
io::endpoint* factory::new_endpoint(
  ...
  if (!absl::SimpleAtoi(find_param(cfg, "length"), &rrd_length)) {
    ...
    log_v2::instance()
        .get(log_v2::CORE)
        ->error(
            "unified_sql: the length field should contain a string containing "
            "a "
    ...
    if (it != cfg.params.end()) {
      if (!absl::SimpleAtob(it->second, &store_in_hosts_services)) {
        log_v2::instance()
            .get(log_v2::CORE)
            ->error(
                "factory: cannot parse the 'store_in_hosts_services' boolean: "
                "the "
                "content is '{}'",
                it->second);
  ...
  log_v2::instance()
      .get(log_v2::CORE)
      ->debug("SQL: store in hosts/services: {}",
              store_in_hosts_services ? "yes" : "no");
  log_v2::instance()
      .get(log_v2::CORE)
      ->debug("SQL: store in resources: {}", store_in_resources ? "yes" : "no");
  ...
  uint32_t instance_timeout = 5 * 60;
  {
    ...
    if (it != cfg.params.end() &&
      ...
      log_v2::instance()
          .get(log_v2::CORE)
          ->error(
              "factory: cannot parse the 'instance_timeout' value. It should "
              "be an "
