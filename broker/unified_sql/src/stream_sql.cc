...
void stream::_process_host_group_member(const std::shared_ptr<io::data>& d) {
  ...
  if (!_host_instance_known(hgm.host_id)) {
    SPDLOG_LOGGER_WARN(_logger_sql,
                       "unified_sql: host {0} not added to hostgroup {1} "
  ...
  if (hgm.enabled) {
    // Log message.
    SPDLOG_LOGGER_INFO(_logger_sql,
                       "unified_sql: enabling membership of host {} to host "
                       "group {} on instance {}",
    ...
    if (_cache_host_instance[hgm.host_id]) {
      if (_hostgroup_cache.find(hgm.group_id) == _hostgroup_cache.end()) {
        SPDLOG_LOGGER_ERROR(_logger_sql,
                            "unified_sql: host group {} does not exist - "
                            "insertion before insertion of "
                            "members",
                            hgm.group_id);
        _prepare_hg_insupdate_statement();
      ...
      SPDLOG_LOGGER_ERROR(
          _logger_sql,
          "unified_sql: host with host_id = {} does not exist - unable to "
          "store "
  ...
  // Delete.
    ...
    // Log message.
    SPDLOG_LOGGER_INFO(_logger_sql,
                       "unified_sql: disabling membership of host {} to host "
                       "group {} on instance {}",
                       hgm.host_id, hgm.group_id, hgm.poller_id);
...
void stream::_process_host(const std::shared_ptr<io::data>& d) {
  ...
  SPDLOG_LOGGER_INFO(
      _logger_sql,
      "unified_sql: processing host event (poller: {}, host: {}, name: {})",
        ...
        if (h.enabled()) {
          ...
          if (found == _resource_cache.end()) {
            ...
            try {
              ...
              SPDLOG_LOGGER_CRITICAL(
                  _logger_sql,
                  "unified_sql: unable to insert new host resource {}: {}",
                  h.host_id(), e.what());
              ...
              try {
                ...
                SPDLOG_LOGGER_CRITICAL(
                    _logger_sql, "No host resource in database with id {}: {}",
                    h.host_id(), e.what());
                return;
              }
            }
            SPDLOG_LOGGER_DEBUG(_logger_sql, "insert resource {} for host{}",
                                res_id, h.host_id());
          }
          if (res_id == 0) {
            res_id = found->second;
            SPDLOG_LOGGER_DEBUG(_logger_sql, "update resource {} for host{}",
                                res_id, h.host_id());
            // UPDATE
          ...
          for (auto& tag : h.tags()) {
            ...
            if (it_tags_cache == _tags_cache.end()) {
              SPDLOG_LOGGER_ERROR(_logger_sql,
                                  "unified_sql: could not find in cache the "
                                  "tag ({}, {}) for host "
                                  "'{}': "
                                  "trying to add it.",
              ...
              try {
                ...
                SPDLOG_LOGGER_ERROR(
                    _logger_sql,
                    "unified_sql: unable to insert new tag ({},{}): {}",
            ...
            if (it_tags_cache != _tags_cache.end()) {
              ...
              SPDLOG_LOGGER_DEBUG(_logger_sql,
                                  "unified_sql: new relation between host "
                                  "(resource_id: {}, host_id: "
                                  "{}) "
                                  "and tag ({},{})",
          ...
          if (found != _resource_cache.end()) {
            ...
            SPDLOG_LOGGER_INFO(
                _logger_sql,
                "unified_sql: no need to remove host {}, it is not in database",
                h.host_id());
          }
        }
      }
...
void stream::_process_pb_instance(const std::shared_ptr<io::data>& d) {
  ...
  //  if (_configurator) {
  //    _logger_sql->info(
  //        "unified_sql: Configurator is updating the database with the poller
  //        " "configuration");
  //  } else {
  // Clean tables.
      ...
      if (s.enabled()) {
        ...
        if (found == _resource_cache.end()) {
          ...
          if (s.severity_id() > 0) {
            ...
            SPDLOG_LOGGER_DEBUG(
                _logger_sql, "service ({}, {}) with severity_id {} => uid = {}",
          ...
          try {
            ...
            SPDLOG_LOGGER_CRITICAL(_logger_sql,
                                   "unified_sql: unable to insert new service "
                                   "resource ({}, {}): {}",
        ...
        for (auto& tag : s.tags()) {
          ...
          if (it_tags_cache == _tags_cache.end()) {
            SPDLOG_LOGGER_ERROR(_logger_sql,
                                "unified_sql: could not find in cache the tag "
                                "({}, {}) for service "
                                "({},{}): trying to add it.",
                                tag.id(), tag.type(), s.host_id(),
                                s.service_id());
            if (!_tag_insert.prepared())
              _tag_insert = _mysql.prepare_query(
                  "INSERT INTO tags (id,type,name) VALUES(?,?,?)");
            ...
            try {
              ...
              SPDLOG_LOGGER_ERROR(
                  _logger_sql,
                  "unified_sql: unable to insert new tag ({},{}): {}", tag.id(),
                  tag.type(), e.what());
            }
          }
          ...
          if (it_tags_cache != _tags_cache.end()) {
            ...
            _resources_tags_insert.bind_value_as_u64(1, res_id);
            SPDLOG_LOGGER_DEBUG(_logger_sql,
                                "unified_sql: new relation between service "
                                "(resource_id: {},  ({}, "
                                "{})) and tag ({},{})",
                                res_id, s.host_id(), s.service_id(), tag.id(),
                                tag.type());
            _mysql.run_statement(
                _resources_tags_insert,
                database::mysql_error::store_tags_resources_tags, conn);
            ...
            SPDLOG_LOGGER_ERROR(_logger_sql,
                                "unified_sql: could not find the tag ({}, {}) "
                                "in cache for host '{}'",
        ...
        if (found != _resource_cache.end()) {
          ...
          SPDLOG_LOGGER_INFO(
              _logger_sql,
              "unified_sql: no need to remove service ({}, {}), it is not in "
              "database",
              s.host_id(), s.service_id());
        }
      }
...
void stream::_process_tag(const std::shared_ptr<io::data>& d) {
  ...
  switch (tg.action()) {
    case Tag_Action_ADD:
      ...
      if (tag_id) {
        SPDLOG_LOGGER_TRACE(
            _logger_sql, "unified_sql: add already existing tag {}", tg.id());
        ...
        SPDLOG_LOGGER_TRACE(_logger_sql, "unified_sql: add tag {}", tg.id());
        ...
        try {
          ...
          SPDLOG_LOGGER_ERROR(
              _logger_sql, "unified sql: unable to insert new tag ({},{}): {}",
    ...
    case Tag_Action_MODIFY:
      SPDLOG_LOGGER_TRACE(_logger_sql, "unified_sql: modify tag {}", tg.id());
      if (cache_ptr) {
        cache_ptr->add_tag(tg.id(), tg.name(), tg.type(), tg.poller_id());
      }
      _tag_update.bind_value_as_u64(0, tg.id());
