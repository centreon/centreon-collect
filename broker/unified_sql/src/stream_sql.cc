      ...
      if (_store_in_resources) {
        ...
        if (h.enabled()) {
          ...
          if (found == _resource_cache.end()) {
            ...
            _resources_host_insert.bind_value_as_bool(5,
                                                      h.downtime_depth() > 0);
            ...
            _resources_host_insert.bind_value_as_bool(
                19, h.notifications_enabled());
            _resources_host_insert.bind_value_as_bool(
                20, h.passive_checks_enabled());
            _resources_host_insert.bind_value_as_bool(
                21, h.active_checks_enabled());
    ...
    if (ss.host_id() && ss.service_id()) {
      ...
      if (_store_in_resources) {
        ...
        if (ss.enabled()) {
          ...
          // INSERT
          if (found == _resource_cache.end()) {
            _resources_service_insert.bind_value_as_u64(0,
    ...
    try {
      ...
      try {
        ...
        if (_mysql.fetch_row(res)) {
          ...
          log_v2::sql()->debug(
              "sql: loaded index {} of ({}, {}) with rrd_len={}, locked='{}' and locked={}", index_id,
              ss.host_id(), ss.service_id(), info.rrd_retention, res.value_as_str(6), res.value_as_bool(6));
          _index_cache[{ss.host_id(), ss.service_id()}] = std::move(info);
          // Create the metric mapping.
          auto im{std::make_shared<storage::index_mapping>(
              info.index_id, ss.host_id(), ss.service_id())};
