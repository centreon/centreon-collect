...
void stream::_process_host_check(const std::shared_ptr<io::data>& d) {
  ...
  if (hc.check_type ||                  // - passive result
    ...
    // Apply to DB.
    SPDLOG_LOGGER_INFO(
        _logger_sql, "SQL: processing host check event (host: {}, command: {}",
...
void stream::_process_host_parent(const std::shared_ptr<io::data>& d) {
  ...
  if (hp.enabled) {
    // Log message.
    SPDLOG_LOGGER_INFO(_logger_sql, "SQL: host {} is parent of host {}",
          ...
          if (found == _resource_cache.end()) {
            ...
            try {
              ...
              SPDLOG_LOGGER_CRITICAL(
                  _logger_sql, "SQL: unable to insert new host resource {}: {}",
                  h.host_id(), e.what());
...
void stream::_process_pb_adaptive_host(const std::shared_ptr<io::data>& d) {
  SPDLOG_LOGGER_INFO(_logger_sql, "SQL: processing pb adaptive host");
  ...
  if (query.size() > buf.size()) {
    ...
    SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: query <<{}>>", query);
    ...
    if (_store_in_resources) {
      ...
      if (res_query.size() > res_buf.size()) {
        ...
        SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: query <<{}>>", res_query);
...
void stream::_process_pb_service(const std::shared_ptr<io::data>& d) {
  ...
  SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: pb service output: <<{}>>",
                      s.output());
  // Processed object.
  if (!_host_instance_known(s.host_id())) {
    SPDLOG_LOGGER_WARN(
...
void stream::_process_pb_adaptive_service(const std::shared_ptr<io::data>& d) {
  SPDLOG_LOGGER_DEBUG(_logger_sql, "SQL: processing pb adaptive service");
  _finish_action(-1, actions::host_parents | actions::comments |
                         actions::downtimes | actions::host_dependencies |
                         actions::service_dependencies);
  // Processed object.
...
void stream::_check_and_update_index_cache(const Service& ss) {
  ...
  if (it_index_cache == _index_cache.end()) {
    ...
    try {
      ...
      SPDLOG_LOGGER_DEBUG(
          _logger_sql, "sql: loaded index {} of ({}, {}) with rrd_len={}",
      ...
      SPDLOG_LOGGER_DEBUG(
          _logger_sql, "sql: cannot insert new index for service ({}, {}): {}",
...
void stream::_process_service_status(const std::shared_ptr<io::data>& d) {
  ...
  _logger_sto->info("SQL: service status output: <<{}>>", ss.output);
  _logger_sto->info("SQL: service status perfdata: <<{}>>", ss.perf_data);
...
void stream::_process_pb_service_status(const std::shared_ptr<io::data>& d) {
  ...
  SPDLOG_LOGGER_DEBUG(
      ...
      sscr.output());
  SPDLOG_LOGGER_DEBUG(
      _logger_sql, "SQL: service ({}, {}) status check result perfdata: <<{}>>",
...
void stream::_process_severity(const std::shared_ptr<io::data>& d) {
  ...
  SPDLOG_LOGGER_DEBUG(_logger_sql, "SQL: processing severity");
  ...
  switch (sv.action()) {
    case Severity_Action_ADD:
      ...
      if (severity_id) {
        ...
        SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: add severity {}", sv.id());
    ...
    case Severity_Action_MODIFY:
      _add_action(conn, actions::severities);
      SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: modify severity {}", sv.id());
      _severity_update.bind_value_as_u64(0, sv.id());
      _severity_update.bind_value_as_u32(1, sv.type());
      _severity_update.bind_value_as_str(2, sv.name());
      _severity_update.bind_value_as_u32(3, sv.level());
...
void stream::_process_tag(const std::shared_ptr<io::data>& d) {
  ...
  SPDLOG_LOGGER_INFO(_logger_sql, "SQL: processing tag");
  _finish_action(-1, actions::tags);
  ...
  switch (tg.action()) {
    case Tag_Action_ADD:
      if (tag_id) {
        SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: add already existing tag {}",
                            tg.id());
        ...
        SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: add tag {}", tg.id());
    ...
    case Tag_Action_MODIFY:
      SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: modify tag {}", tg.id());
      _tag_update.bind_value_as_u64(0, tg.id());
      _tag_update.bind_value_as_u32(1, tg.type());
      _tag_update.bind_value_as_str(2, tg.name());
      if (tag_id) {
    ...
    case Tag_Action_DELETE: {
      ...
      if (it != _tags_cache.end()) {
        ...
        SPDLOG_LOGGER_TRACE(_logger_sql, "SQL: delete tag {}", id);
