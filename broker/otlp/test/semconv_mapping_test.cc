...
/* Rules exercised by the tests below. The broker ships no table of its own:
 * without a mapping file every metric goes under centreon.*. */
constexpr std::string_view k_test_table = R"({
  "metrics": {
    "cpu.utilization.percentage": {
      "name": "system.cpu.utilization", "unit": "1", "scale": 0.01
    },
    "cpu.user.percentage": {
      "name": "system.cpu.utilization", "unit": "1", "scale": 0.01,
    ...
    "interface.traffic.in.bitspersecond": {
      "name": "centreon.network.throughput", "unit": "By/s", "scale": 0.125,
      "attributes": { "network.io.direction": "receive" },
      "instance_attribute": "network.interface.name"
    },
    "memory.usage.bytes": {
      "name": "system.memory.usage", "unit": "By",
    ...
    "interface.packets.in.count": {
      "name": "system.network.packet.count", "unit": "{packet}",
...
mapping map_test(std::string_view perfdata_name,
  ...
  static const mapping_table::pointer table =
      mapping_table::from_json(k_test_table);
  return map_metric(perfdata_name, unit, value_type, *table);
}
...
TEST(otlp_mapping, percent_becomes_ratio) {
  auto m = map_test("cpu.utilization.percentage", "%", perfdata::gauge);
...
TEST(otlp_mapping, milliseconds_become_seconds) {
  auto m = map_test("rta", "ms", perfdata::gauge);
...
TEST(otlp_mapping, bits_per_second_become_bytes_per_second) {
  auto m = map_test("eth0#interface.traffic.in.bitspersecond", "b/s",
                      perfdata::gauge);
...
TEST(otlp_mapping, bytes_are_not_rescaled) {
  auto m = map_test("memory.usage.bytes", "B", perfdata::gauge);
...
TEST(otlp_mapping, cpu_mode_attribute_is_current_spelling) {
  auto m = map_test("cpu.user.percentage", "%", perfdata::gauge);
...
TEST(otlp_mapping, mountpoint_comes_from_the_instance_part) {
  auto m = map_test("/var#disk.space.usage.bytes", "B", perfdata::gauge);
...
TEST(otlp_mapping, interface_name_comes_from_the_instance_part) {
  auto m = map_test("eth0#interface.packets.in.count", "c",
                      perfdata::counter);
...
TEST(otlp_mapping, missing_required_instance_degrades_to_fallback) {
  auto m = map_test("disk.space.usage.bytes", "B", perfdata::gauge);
...
TEST(otlp_mapping, unknown_metric_falls_back_to_centreon_namespace) {
  auto m = map_test("my_custom_check", "queries", perfdata::gauge);
  EXPECT_TRUE(m.is_fallback);
...
TEST(otlp_mapping, fallback_keeps_instance_as_attribute) {
  auto m = map_test("queue1#custom.depth", "", perfdata::gauge);
  EXPECT_TRUE(m.is_fallback);
...
TEST(otlp_mapping, counter_fallback_is_a_monotonic_sum) {
  auto m = map_test("weird_counter", "c", perfdata::counter);
  EXPECT_EQ(m.instr, instrument::sum_monotonic);
  auto g = map_test("weird_gauge", "", perfdata::gauge);
