 ...
 *
 * The neb module registers the service and host status events this module
 * consumes.
 *
...
bool broker_module_deinit() {
  ...
  if (!--instances)
    io::protocols::instance().unreg("OTLP");
...
void broker_module_init(void const* arg [[maybe_unused]]) {
  ...
  if (!instances++) {
    SPDLOG_LOGGER_INFO(logger, "otlp: module for Centreon Broker {}",
                       CENTREON_BROKER_VERSION);

    /* The consumed events belong to neb and are registered by 10-neb.so,
     * declared as a parent above. */
    io::protocols::instance().reg("OTLP", std::make_shared<otlp::factory>(), 1,
                                  7);
  }
}
}
