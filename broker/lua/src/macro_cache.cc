...
const std::string& macro_cache::get_host_group_name(uint64_t id) const {
  ...
  if (found->second.first->type() == neb::host_group::static_type())
    return std::static_pointer_cast<neb::host_group>(found->second.first)->name;
  else
    return std::static_pointer_cast<neb::pb_host_group>(found->second.first)
        ->obj()
        .name();
}
...
std::string const& macro_cache::get_service_group_name(uint64_t id) const {
  ...
  if (found->second.first->type() == neb::service_group::static_type())
    return std::static_pointer_cast<neb::service_group>(found->second.first)
        ->name;
  else
    return std::static_pointer_cast<neb::pb_service_group>(found->second.first)
        ->obj()
        .name();
}
...
void macro_cache::_process_pb_host_group(
  ...
  if (hg.enabled()) {
    ...
    if (found != _host_groups.end()) {
      ...
      absl::flat_hash_set<uint32_t> pollers{hg.poller_id()};
      _host_groups[hg.hostgroup_id()] = std::make_pair(data, pollers);
    }
  } else {
    /* We check that no more pollers need this host group. So if the set is
     * empty, we can also remove the host group. */
...
void macro_cache::_process_service_group(
  ...
  if (sg->enabled) {
    ...
    if (found != _service_groups.end()) {
      ...
      std::static_pointer_cast<neb::service_group>(found->second.first)->name =
          sg->name;
    } else {
      /* Here, we add the servicegroup and the first poller that needs it */
      absl::flat_hash_set<uint32_t> pollers{sg->poller_id};
      _service_groups[sg->id] = std::make_pair(data, pollers);
    }
  } else {
...
void macro_cache::_process_pb_service_group(
  ...
  if (sg.enabled()) {
    ...
    if (found != _service_groups.end()) {
      found->second.second.insert(sg.poller_id());
      ServiceGroup& current_sg =
          std::static_pointer_cast<neb::pb_service_group>(found->second.first)
              ->mut_obj();
      current_sg.set_name(sg.name());
    } else {
      /* Here, we add the servicegroup and the first poller that needs it */
      absl::flat_hash_set<uint32_t> pollers{sg.poller_id()};
      _service_groups[sg.servicegroup_id()] = std::make_pair(data, pollers);
    }
  } else {
    /* We check that no more pollers need this service group. So if the set is
     * empty, we can also remove the service group. */
...
void macro_cache::_save_to_disk() {
  ...
  for (auto it = _host_groups.begin(), end = _host_groups.end(); it != end;
       ++it) {
    if (it->second.first->type() == neb::host_group::static_type()) {
      for (auto poller_id : it->second.second) {
        std::static_pointer_cast<neb::host_group>(it->second.first)->poller_id =
            poller_id;
        _cache->add(it->second.first);
      }
    } else {
      for (auto poller_id : it->second.second) {
        std::static_pointer_cast<neb::pb_host_group>(it->second.first)
            ->mut_obj()
            .set_poller_id(poller_id);
        _cache->add(it->second.first);
      }
    }
  }
  ...
  for (auto it = _service_groups.begin(), end = _service_groups.end();
       it != end; ++it) {
    if (it->second.first->type() == neb::service_group::static_type()) {
      for (auto poller_id : it->second.second) {
        std::static_pointer_cast<neb::service_group>(it->second.first)
            ->poller_id = poller_id;
        _cache->add(it->second.first);
      }
    } else {
      for (auto poller_id : it->second.second) {
        std::static_pointer_cast<neb::pb_service_group>(it->second.first)
            ->mut_obj()
            .set_poller_id(poller_id);
        _cache->add(it->second.first);
      }
