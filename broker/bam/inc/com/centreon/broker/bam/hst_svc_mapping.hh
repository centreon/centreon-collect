...
class hst_svc_mapping {
 ...
 public:
  ...
  uint64_t get_host_id(std::string const& hst) const;
  std::pair<uint64_t, uint64_t> get_service_id(std::string const& hst,
                                               std::string const& svc) const;
  void set_host(std::string const& hst, uint64_t host_id);
  ...
  bool get_activated(uint64_t hst_id, uint64_t service_id) const;
};
}  // namespace com::centreon::broker::bam
