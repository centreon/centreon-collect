...
def remove_useless_json():
    ...
    while i < len(data):
        ...
        if debug:
            print("poller id ", data[i]
                  ["centreonBroker"]["poller_id"], protocols)
    ...
    for i in range(0, len(data)):
        if "output" in data[i]["centreonBroker"]:
            for output_item in data[i]["centreonBroker"]["output"]:
                ...
                output_tmp.add_poller_id(
                    data[i]["centreonBroker"]["poller_id"])
                output_tmp.add_broker_id(
                    data[i]["centreonBroker"]["broker_id"])
