    ...
    node("C++") {
      dir('centreon-collect-centos7') {
        ...
        withSonarQubeEnv('SonarQubeDev') {
          sh 'printenv'
          if (env.CHANGE_ID) {
            sh 'docker run -i --entrypoint /src/ci/scripts/collect-sources-analysis.sh -v "$PWD:/src" registry.centreon.com/centreon-collect-centos7-dependencies:22.04-testdocker "PR" "$SONAR_AUTH_TOKEN" "$SONAR_HOST_URL" "$CHANGE_BRANCH" "$CHANGE_TARGET" "$CHANGE_ID"'
          } else {
