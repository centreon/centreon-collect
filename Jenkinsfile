    ...
    node("C++") {
      dir('centreon-collect-debian11') {
        ...
        withSonarQubeEnv('SonarQubeDev') {
          sh 'docker run -i -v "$PWD:/src" -w="/src" --entrypoint /src/ci/scripts/collect-sources-analysis.sh --rm -u $(id -u):$(id -g) sonarsource/sonar-scanner-cli:latest "$SONAR_AUTH_TOKEN" "$SONAR_HOST_URL"'
        }    
      }
    }
  },
