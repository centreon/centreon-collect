...
class service : public object {
 public:
  ...
  opt<int> const& acknowledgement_type() const noexcept;
  opt<bool> const& active_checks_enabled() const noexcept;
  opt<std::string> const& check_command() const noexcept;
  opt<double> const& check_execution_time() const noexcept;
  opt<int> const& check_flapping_recovery_notification() const noexcept;
  opt<double> const& check_latency() const noexcept;
  opt<int> const& check_options() const noexcept;
  opt<std::string> const& check_period() const noexcept;
  opt<int> const& check_type() const noexcept;
  opt<int> const& current_attempt() const noexcept;
  opt<uint64_t> const& current_event_id() const noexcept;
  opt<uint64_t> const& current_notification_id() const noexcept;
  opt<int> const& current_notification_number() const noexcept;
  opt<uint64_t> const& current_problem_id() const noexcept;
  opt<int> const& current_state() const noexcept;
  map_customvar const& customvariables() const noexcept;
  opt<std::string> const& event_handler() const noexcept;
  opt<bool> const& event_handler_enabled() const noexcept;
  opt<bool> const& flap_detection_enabled() const noexcept;
  opt<bool> const& has_been_checked() const noexcept;
  uint64_t host_id() const noexcept;
  std::string const& host_name() const noexcept;
  opt<bool> const& is_flapping() const noexcept;
  opt<uint64_t> const& flapping_comment_id() const noexcept;
  opt<uint64_t> const& acknowledgement_comment_id() const noexcept;
  opt<time_t> const& last_acknowledgement() const noexcept;
  opt<time_t> const& last_check() const noexcept;
  opt<uint64_t> const& last_event_id() const noexcept;
  opt<time_t> const& last_hard_state() const noexcept;
  opt<time_t> const& last_hard_state_change() const noexcept;
  opt<time_t> const& last_notification() const noexcept;
  opt<uint64_t> const& last_problem_id() const noexcept;
  opt<time_t> const& last_state() const noexcept;
  opt<time_t> const& last_state_change() const noexcept;
  opt<time_t> const& last_time_critical() const noexcept;
  opt<time_t> const& last_time_ok() const noexcept;
  opt<time_t> const& last_time_unknown() const noexcept;
  opt<time_t> const& last_time_warning() const noexcept;
  opt<std::string> const& long_plugin_output() const noexcept;
  opt<unsigned int> const& max_attempts() const noexcept;
  opt<unsigned long> const& modified_attributes() const noexcept;
  opt<time_t> const& next_check() const noexcept;
  opt<unsigned int> const& normal_check_interval() const noexcept;
  opt<std::string> const& notification_period() const noexcept;
  opt<bool> const& notifications_enabled() const noexcept;
  opt<bool> const& notified_on_critical() const noexcept;
  opt<bool> const& notified_on_unknown() const noexcept;
  opt<bool> const& notified_on_warning() const noexcept;
  opt<int> const& obsess_over_service() const noexcept;
  opt<bool> const& passive_checks_enabled() const noexcept;
  opt<double> const& percent_state_change() const noexcept;
  opt<std::string> const& performance_data() const noexcept;
  opt<std::string> const& plugin_output() const noexcept;
  opt<bool> const& problem_has_been_acknowledged() const noexcept;
  opt<int> const& process_performance_data() const noexcept;
  opt<unsigned int> const& retry_check_interval() const noexcept;
  uint64_t service_id() const noexcept;
  std::string const& service_description() const noexcept;
  opt<std::vector<int> > const& state_history() const noexcept;
  opt<int> const& state_type() const noexcept;
