...
// don't put it in precomp.hpp because it includes time.h to all sources (not
// compatible with custom gettimeofday in utils.cc)
...
class setter_base {
 protected:
  const absl::string_view _field_name;

 public:
  setter_base(const absl::string_view& field_name) : _field_name(field_name) {}

  virtual ~setter_base() = default;
  virtual bool apply_from_cfg(state& obj, const char* value) = 0;
  virtual bool apply_from_json(state& obj, const nlohmann::json& doc) = 0;
};
...
class state {
 public:
  ...
  using setter_map =
      absl::flat_hash_map<absl::string_view, std::unique_ptr<setter_base>>;
