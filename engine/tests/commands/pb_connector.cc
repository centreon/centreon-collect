...
class PbConnector : public ::testing::Test {
 public:
  void SetUp() override {
...
TEST_F(PbConnector, NewConnector) {
  ASSERT_THROW(new commands::connector("", "bar"), std::exception);
}

TEST_F(PbConnector, ForwardWithoutName) {
...
TEST_F(PbConnector, ForwardWithoutCmd) {
  auto c = std::make_shared<commands::connector>(
      "test segfault", "tests/bin_connector_test_run --kill=2");
...
// Then it returns a NULL pointer.
TEST_F(PbConnector, SimpleConnector) {
...
TEST_F(PbConnector, RunWithTimeout) {
...
TEST_F(PbConnector, RunConnectorAsync) {
...
TEST_F(PbConnector, RunWithConnectorSwitchedOff) {
...
TEST_F(PbConnector, RunConnectorSetCommandLine) {
  my_listener lstnr;
  nagios_macros macros = nagios_macros();
