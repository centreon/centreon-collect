...
class PbApplierSeverity : public ::testing::Test {
 public:
...
TEST_F(PbApplierSeverity, AddSeverityFromConfig) {
...
// Then the applier add_object adds the severity in the configuration set
// and in the severities map.
TEST_F(PbApplierSeverity, PbAddSeverityFromConfig) {
  configuration::applier::severity aply;
  configuration::Severity sv;
...
TEST_F(PbApplierSeverity, ModifySeverityFromConfig) {
  configuration::applier::severity aply;
  configuration::severity sv;
  sv.parse("severity_id", "1");
  sv.parse("severity_type", "service");
...
TEST_F(PbApplierSeverity, PbModifySeverityFromConfig) {
  configuration::applier::severity aply;
  configuration::Severity sv;
  configuration::severity_helper sv_hlp(&sv);
  sv_hlp.hook("severity_id", "1");
...
TEST_F(PbApplierSeverity, RemoveSeverityFromConfig) {
  configuration::applier::severity aply;
  configuration::severity sv;
  sv.parse("severity_id", "1");
  sv.parse("severity_type", "service");
...
// This configuration is added and then removed.
// There is no more severity.
TEST_F(PbApplierSeverity, PbRemoveSeverityFromConfig) {
  configuration::applier::severity aply;
  configuration::Severity sv;
  configuration::severity_helper sv_hlp(&sv);
  sv_hlp.hook("severity_id", "1");
