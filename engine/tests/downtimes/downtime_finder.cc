...
class DowntimeFinderFindMatchingAllTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    hst.parse("_HOST_ID", std::to_string(18).c_str());
    configuration::applier::host hst_aply;
    hst_aply.add_object(hst);

    downtime_manager::instance().clear_scheduled_downtimes();
    downtime_manager::instance().initialize_downtime_data();
    new_downtime(0, 18, 0, 234567891, 134567892, 1, 0, 84, "other_author",
                 "test_comment");
    //    new_downtime(1, "first_host", "test_service", 123456789, 134567892, 1,
    //    0,
    //                 42, "test_author", "other_comment");
    //    new_downtime(2, "other_host", "other_service", 123456789, 345678921,
    //    0, 1,
    //                 42, "", "test_comment");
    //    new_downtime(3, "test_host", "test_service", 234567891, 345678921, 0,
    //    1, 84,
    //                 "test_author", "");
    //    new_downtime(4, "other_host", "test_service", 123456789, 134567892, 1,
    //    1,
    //                 42, "test_author", "test_comment");
    //    new_downtime(5, "out_host", "out_service", 7265943625, 7297479625, 1,
    //    2,
...
///"anyauthor") / Then an empty result_set is returned
...
///"4102441200") / Then all downtimes with 4102441200 as end time are returned
