...
class open_telemetry
    : public com::centreon::engine::modules::otl_server::open_telemetry {
 protected:
 ...
 public:
  open_telemetry(const std::string_view config_file_path,
      ...
      : com::centreon::engine::modules::otl_server::open_telemetry(
            config_file_path,
            io_context) {}
  ...
  static std::shared_ptr<open_telemetry> load(
    ...
    std::shared_ptr<open_telemetry> ret =
        std::make_shared<open_telemetry>(config_path, io_context);
    ret->_reload();
...
void open_telemetry_test::SetUpTestSuite() {
  ...
  conf_file.close();
  log_v2::otl()->set_level(spdlog::level::trace);
}
