...
grpc::Status engine_impl::RemoveServiceAcknowledgement(
  ...
  auto fn = std::packaged_task<int32_t(void)>([&err, request]() -> int32_t {
    ...
    // FIXME DBO: This command could be optimized, we just change
    // the flag concerning acknowledgement.
  ...
  auto fn = std::packaged_task<int32_t(void)>([&err, request]() -> int32_t {
    ...
    if (request->notify())
      temp_service->notify(notifier::reason_acknowledgement,
                           ...
                           notifier::notification_option_none);
    /* update the status log with the service info */
    // FIXME DBO: we use CHECK_RESULT because it embeds info about
    // acknowledgement but we could make something even smaller.
    temp_service->update_status(service::CHECK_RESULT);
