 ...
 */
void applier::contact::modify_object(const configuration::Contact& obj) {
  // Logging.
  log_v2::config()->debug("Modifying contact '{}'.", obj.contact_name());

  // Find old configuration.
  set_contact::iterator it_cfg(config->contacts_find(obj.contact_name()));
  ...
  configuration::Contact old_cfg;
  old_cfg.CopyFrom(obj);
  old_cfg.Swap(&pb_config.mutable_contacts()->at(obj.contact_name()));
  ...
  if (!MessageDifferencer::Equals(obj.host_notification_commands(),
                                  old_cfg.host_notification_commands())) {
    c->get_host_notification_commands().clear();

    for (auto& cfg_c : obj.host_notification_commands().data()) {
      ...
      if (itt != commands::command::commands.end())
        ...
        throw engine_error() << fmt::format(
            ...
            cfg_c, obj.contact_name());
    }
  }

  // Service notification commands.
  if (!MessageDifferencer::Equals(obj.service_notification_commands(),
                                  old_cfg.service_notification_commands())) {
    c->get_service_notification_commands().clear();

    for (auto& cfg_c : obj.service_notification_commands().data()) {
      command_map::const_iterator itt = commands::command::commands.find(cfg_c);
      if (itt != commands::command::commands.end())
  ...
  for (auto& cfg_cv : obj.customvariables()) {
  ...
  if (obj.customvariables().size() != c->get_custom_variables().size()) {
