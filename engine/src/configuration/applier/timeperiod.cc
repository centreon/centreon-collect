...
void applier::timeperiod::add_object(const configuration::Timeperiod& obj) {
  ...
  if (contains_illegal_object_chars(obj.timeperiod_name()))
    throw engine_error() << fmt::format(
        "Could not register time period '{}': it contains one or more illegal "
        "characters",
        obj.timeperiod_name());
...
void applier::timeperiod::modify_object(
  ...
  com::centreon::common::timeperiods::timeperiod* tp(it_obj->second.get());
...
void applier::timeperiod::resolve_object(const configuration::Timeperiod& obj,
  ...
  // Find time period.
  timeperiod_map::iterator it = ::timeperiods.find(obj.timeperiod_name());
  if (::timeperiods.end() == it || !it->second)
