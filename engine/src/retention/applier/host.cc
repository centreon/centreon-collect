 ...
 *                                   outdated.
 */
void applier::host::apply(const configuration::State& config,
                          list_host const& lst,
                          bool scheduling_info_is_ok) {
  for (list_host::const_iterator it(lst.begin()), end(lst.end()); it != end;
       ++it) {
    try {
      com::centreon::engine::host& hst(find_host((*it)->host_id()));
 ...
 *                                        not outdated.
 */
void applier::host::_update(const configuration::State& config,
  ...
  if (obj.get_retain_status_information()) {
    ...
    if (state.last_time_up().has_value())
      obj.set_last_time_up(*state.last_time_up());
    if (state.last_time_down().has_value())
      obj.set_last_time_down(*state.last_time_down());
    if (state.last_time_unreachable().has_value())
  ...
  if (obj.get_retain_nonstatus_information()) {
    ...
    if (state.obsess_over_host().has_value() &&
