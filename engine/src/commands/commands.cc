...
int cmd_schedule_check(int cmd, std::string_view args) {
  ...
  if (cmd == CMD_SCHEDULE_HOST_CHECK || cmd == CMD_SCHEDULE_FORCED_HOST_CHECK)
    temp_host->schedule_check(delay_time, cmd == CMD_SCHEDULE_FORCED_HOST_CHECK
  ...
  switch (cmd) {
    ...
    case CMD_CHANGE_NORMAL_SVC_CHECK_INTERVAL:
      ...
      if (old_dval == 0 && found_svc->second->active_checks_enabled() &&
        ...
        /* schedule a check for right now (or as soon as possible) */
        time(&preferred_time);
        if (found_svc->second->check_period_ptr &&
            !found_svc->second->check_period_ptr->check_time_against_period(
                preferred_time)) {
          next_valid_time =
...
int cmd_change_object_char_var(int cmd, std::string_view args) {
  ...
  switch (cmd) {
    ...
    case CMD_CHANGE_CONTACT_SVC_NOTIFICATION_TIMEPERIOD:
      ...
      found = ::timeperiods.find(temp_ptr);
...
void acknowledge_host_problem(host* hst,
  ...
  /* send data to event broker */
  broker_acknowledgement_data(hst, ack_author, ack_data, type, notify,
                              persistent);
...
void acknowledge_service_problem(service* svc,
  ...
  /* send data to event broker */
  broker_acknowledgement_data(svc, ack_author, ack_data, type, notify,
                              persistent);
