...
# HOST_GRPC_PORT: 50155 falls inside this Windows machine's Hyper-V dynamic
# port exclusion range (50060-50159, confirmed via
# `netsh interface ipv4 show excludedportrange protocol=tcp`) - Docker
# Desktop's port-forwarding proxy cannot bind ANY host port in that range
# regardless of image/packaging approach. Local-only workaround: remap just
# the host side to an unaffected port; the container still listens on the
# real 50155 internally, exactly as production does.
HOST_GRPC_PORT=19155
CREATE_EXTRA_ARGS="-p ${HOST_GRPC_PORT}:50155" create_with_configs centreon-engine-wiring-$$
...
for _ in $(seq 1 15); do
  grpc_output=$("$BUF_BIN" curl --schema "$ENGINE_PROTO_DIR" --protocol grpc --http2-prior-knowledge \
    http://127.0.0.1:${HOST_GRPC_PORT}/com.centreon.engine.Engine/GetVersion 2>&1) && break
  sleep 1
done
echo "$grpc_output"
