...
class task_manager {
  struct internal_task {
    ...
    internal_task(task* tsk, uint64_t id, uint32_t interval, bool is_runnable,
                  bool should_delete)
        : id{id},
 ...
 public:
  ...
  uint64_t add(task* t, timestamp const& when, bool is_runnable = false,
               bool should_delete = false);

  uint64_t add(task* t, timestamp const& when, uint32_t interval,
               bool is_runnable = false, bool should_delete = false);
