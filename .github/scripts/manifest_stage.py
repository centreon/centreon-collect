#!/usr/bin/env python3
"""Manifest-driven staging tool for package-less Docker builds.

Reads the `contents:` list of one or more nfpm yaml manifests (the same
files nfpm itself packages from) and copies every entry's `src` to
<root>/<dst> - mechanically replaying what nfpm would put in the
.deb/.rpm, no new path logic, no guessing.

Deliberately dumb: unrecognized entry shapes (a content type family we
don't recognize, a brace-glob with zero expansions, a glob matching zero
files) are HARD FAILURES. A silent skip here is exactly the failure mode
this tool exists to eliminate - if the manifest says a file should be
there, either it lands, or the tool stops and says why not.
"""
import argparse
import glob
import os
...
KNOWN_TYPE_FAMILIES = {"file", "config", "dir", "ghost"}
...
def do_stage(yaml_paths, packager, staging_root):
...
def main():
    ...
    ap.add_argument("--root", required=True, help="staging output directory")
    args = ap.parse_args()
    do_stage(args.manifest, args.packager, args.root)
