...
bool timeperiod_helper::hook(std::string_view key,
  ...
  if (key == "exclude") {
    ...
    retval = get_timerange(value, obj->mutable_timeranges()->mutable_sunday());
  else if (key == "monday")
    retval = get_timerange(value, obj->mutable_timeranges()->mutable_monday());
  else if (key == "tuesday")
    retval = get_timerange(value, obj->mutable_timeranges()->mutable_tuesday());
    ...
    retval = get_timerange(value, obj->mutable_timeranges()->mutable_friday());
  ...
  if (!retval) {
    ...
    return _add_calendar_date(line) || _add_other_date(line);
  }
  return retval;
}

bool timeperiod_helper::hook(const std::string& line) {
  ...
  if (hook(key, value) || _add_calendar_date(line) || _add_other_date(line))
    return true;
  return false;
}
