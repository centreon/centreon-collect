...
SPDLOG_INLINE centreon_rotating_file_sink<Mutex>::centreon_rotating_file_sink(
...
centreon_rotating_file_sink<Mutex>::calc_filename(const filename_t& filename,
                                                  std::size_t index) {
  if (index == 0u) {
    return filename;
  }
...
template <typename Mutex>
SPDLOG_INLINE filename_t centreon_rotating_file_sink<Mutex>::filename() {
  std::lock_guard<Mutex> lock(base_sink<Mutex>::mutex_);
  return file_helper_.filename();
}

template <typename Mutex>
SPDLOG_INLINE void centreon_rotating_file_sink<Mutex>::rotate_now() {
  std::lock_guard<Mutex> lock(base_sink<Mutex>::mutex_);
  rotate_();
}
 ...
 */
template <typename Mutex>
SPDLOG_INLINE bool centreon_rotating_file_sink<Mutex>::set_filename(
...
template <typename Mutex>
SPDLOG_INLINE void centreon_rotating_file_sink<Mutex>::sink_it_(
...
template <typename Mutex>
SPDLOG_INLINE void centreon_rotating_file_sink<Mutex>::flush_() {
  file_helper_.flush();
}
...
// log.3.txt -> delete
template <typename Mutex>
SPDLOG_INLINE void centreon_rotating_file_sink<Mutex>::rotate_() {
  ...
  for (auto i = max_files_; i > 0; --i) {
    ...
    if (!rename_file_(src, target)) {
      ...
      if (!rename_file_(src, target)) {
        ...
        current_size_ = 0;
        throw_spdlog_ex("centreon_rotating_file_sink: failed renaming " +
...
template <typename Mutex>
SPDLOG_INLINE bool centreon_rotating_file_sink<Mutex>::rename_file_(
