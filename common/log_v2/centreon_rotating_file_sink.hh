...
using centreon_rotating_file_sink_mt = centreon_rotating_file_sink<std::mutex>;
using centreon_rotating_file_sink_st =
    centreon_rotating_file_sink<details::null_mutex>;
...
inline std::shared_ptr<logger> centreon_rotating_logger_mt(
    ...
    bool rotate_on_open = false,
    const file_event_handlers& event_handlers = {}) {
  return Factory::template create<sinks::centreon_rotating_file_sink_mt>(
      logger_name, filename, max_file_size, max_files, rotate_on_open,
      event_handlers);
}

template <typename Factory = spdlog::synchronous_factory>
inline std::shared_ptr<logger> centreon_rotating_logger_st(
    const std::string& logger_name,
    const filename_t& filename,
    size_t max_file_size,
    size_t max_files,
    bool rotate_on_open = false,
    const file_event_handlers& event_handlers = {}) {
  return Factory::template create<sinks::centreon_rotating_file_sink_st>(
      logger_name, filename, max_file_size, max_files, rotate_on_open,
      event_handlers);
}
}  // namespace spdlog
