...
TEST_P(http_server_test, many_request_by_connection) {
  ...
  server = http_server::load(g_io_context, logger, server_conf,
                             std::move(server_creator));

  http_client::pointer client =
      http_client::load(g_io_context, logger, client_conf, client_creator);
