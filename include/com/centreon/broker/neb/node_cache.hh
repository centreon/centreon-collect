...
class node_cache {
 public:
  ...
  void serialize(std::shared_ptr<persistent_cache> cache);
  node_id get_node_by_names(std::string const& host_name,
 ...
 private:
  ...
  std::unordered_map<node_id, neb::service> _services;
  std::unordered_map<node_id, neb::host_status> _host_statuses;
  std::unordered_map<node_id, neb::service_status> _service_statuses;
  std::unordered_map<std::pair<std::string, std::string>, node_id>
      _names_to_node;
  std::unordered_map<node_id, unsigned short> _previous_statuses;

  void _process_host(neb::host const& hst);
  void _process_service(neb::service const& svc);
