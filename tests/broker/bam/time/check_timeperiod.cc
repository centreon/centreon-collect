...
static void parse_file(char const* filename, options& opt) {
  ...
  while (stream.good()) {
    ...
    if (pos == std::string::npos)
      throw msg_fmt(
          "parsing of file '{}' failed because of line: {}", filename, line);
  ...
  if (!opt.preferred_time || !opt.current_time || !opt.ref_time ||
      !opt.period.size())
    throw msg_fmt(
        "invalid configuration file: not all required parameters are set");
}
...
bool checkPeriod(char const* file) {
  try {
    ...
    if (valid != opt.ref_time) {
      ...
      throw msg_fmt(
          "next valid time of timeperiod is {} but does not match reference "
          "time {}",
          valid_str,
          ref_str);
    }
