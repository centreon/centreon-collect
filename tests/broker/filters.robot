...
BFC1
	...
	Start Broker
        ${content}=	Create List	'foo' is not a known category: core: cannot find event category 'foo'	'bar' is not a known category: core: cannot find event category 'bar'	Filters applied on endpoint: neb
...
BFC2
	...
	Config Broker	central
        ...
        ${start}=	Get Current Date
	Start Broker
        ${content}=	Create List	'doe' is not a known category: core: cannot find event category 'doe'	'bar' is not a known category: core: cannot find event category 'bar'	Filters applied on endpoint: all
        ${result}=	Find In Log With Timeout	${centralLog}	${start}	${content}	30
        Should Be True	${result}	msg="Only neb filter should be applied on sql output"
