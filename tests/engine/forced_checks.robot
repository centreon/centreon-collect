...
*** Test Cases ***
EFHC1
    [Documentation] on bbdo2 Engine is configured with hosts and we force checks on one host 5 times and then we check that engine recognizes the force checks.
...
EFHC2
    [Documentation]  on bbdo2 Engine is configured with hosts and we force checks on one host 5 times, engine should recognize the force checks.
    [Tags]    engine    external_cmd    log-v2
    Config Engine    ${1}
    Config Broker    central
    Config Broker    rrd
