...
static void query_execute(char const* q) {
  ...
  if (startptr == endptr)
    throw basic_error_1("invalid query execute: invalid command_id");
  startptr = endptr + 1;
  uint32_t timeout(strtol(startptr, &endptr, 10));
  if (startptr == endptr)
    throw basic_error_1("invalid query execute: invalid is_executed");
  startptr = endptr + 1;
  char const* cendptr(startptr);
  while (isdigit(*cendptr))
    ++cendptr;
  if (startptr == cendptr)
    throw basic_error_1("invalid query execute: invalid exit_code");
  ...
  if (write(STDOUT_FILENO, data.c_str(), data.size()) !=
    ...
    throw basic_error_1("write query execute failed");
  }
}
...
static void query_quit(char const* q) {
  ...
  if (write(STDOUT_FILENO, data.c_str(), data.size()) !=
      static_cast<ssize_t>(data.size()))
    throw basic_error_1("write query quit failed");
  exit(EXIT_SUCCESS);
}
...
static void query_version(char const* q) {
  ...
  if (write(STDOUT_FILENO, data.c_str(), data.size()) !=
      static_cast<ssize_t>(data.size())) {
    throw basic_error_1("write query version failed");
...
static int wait(std::string& query) {
  ...
  if (query_str == endptr) {
    responses.pop_front();
    throw basic_error_1("invalid query");
  }
  query.clear();
