...
BRRDCDDMID1
    ...
    ${result}=    Ctn Find In Log With Timeout    ${centralLog}    ${start}    ${content}    30
    Should Be True    ${result}    A message telling indexes nor metrics appear in the storage database should appear.
...
BRRDCDRB1
    ...
    FOR    ${m}    IN    @{metrics}
        ...
        Should Be True    ${result}    Data before RRD rebuild contain alternatively the metric ID and 0. The expected average is metric_id / 2.
    END
