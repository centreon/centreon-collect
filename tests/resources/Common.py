...
def ctn_run_env():
    ...
    try:
        ...
        logger.console(f"cannot read /proc/1/environ: {e}")
    ...
    if not ctn_run_env():
        if os.path.exists("/usr/bin/systemctl"):
            ...
            if os.path.exists("/usr/libexec/mysqldtoto"):
                ...
                logger.console(
                    "Cannot detect which database is used, trying to start mariadb with service")
                getoutput("service mariadb start")
        else:
...
def ctn_check_resource_ids(typ: str, logfile: str):
    ...
    r_add = re.compile(
        rf".*{types[typ][1]} resource with id (\d+):(\d+) has resource_id (\d+)")
    r_del = re.compile(
        rf".*Disabling {types[typ][2]} resource with id (\d+):(\d+)")
    from_logs = {}
    for line in lines:
        m = r_add.match(line)
        if m:
            ...
            logger.console(
                f"New resource from logs {(parent_id, id)}, resource_id={resource_id}")
            ...
            if m:
                ...
                if (parent_id, id) in from_logs:
                    logger.console(
                        f"Resource {(parent_id, id)} deleted, removing it from list")
    ...
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT resource_id, id, parent_id FROM resources WHERE enabled=1 AND type={types[typ][0]}")
            result = cursor.fetchall()
    ...
    if (len(from_logs) != len(result)):
        logger.console(
            f"Number of resources in logs ({len(from_logs)}) is different from number of resources in database ({len(result)})")
