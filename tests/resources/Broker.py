...
def create_metrics(count: int):
    ...
    with connection:
        with connection.cursor() as cursor:
            ids_db = _extracted_from_create_metrics_27(
...
# TODO Rename this here and in `create_metrics`
def _extracted_from_create_metrics(cursor, connection, count):
    sql = "DELETE FROM metrics"
    cursor.execute(sql)
    connection.commit()
    ids_index = _extracted_from_create_metrics_27(
...
# TODO Rename this here and in `create_metrics`
def _extracted_from_create_metrics_27(arg0, cursor, arg2):
    sql = arg0
    cursor.execute(sql)
    result = cursor.fetchall()
...
# TODO Rename this here and in `remove_graphs_from_db`
