...
def ctn_remove_graphs(port, indexes, metrics, timeout=10):
    ...
    while time.time() < limit:
        ...
        with grpc.insecure_channel("127.0.0.1:{}".format(port)) as channel:
            ...
            try:
                stub.RemoveGraphs(trm, timeout=max(1, limit - time.time()))
                break
            except Exception:
...
def ctn_rebuild_rrd_graphs(port, indexes, timeout: int = TIMEOUT):
    ...
    while time.time() < limit:
        ...
        with grpc.insecure_channel("127.0.0.1:{}".format(port)) as channel:
            ...
            try:
                stub.RebuildRRDGraphs(idx, timeout=max(1, limit - time.time()))
                break
            except Exception:
                logger.console("gRPC server not ready")
...
def ctn_remove_poller(port, name, timeout=TIMEOUT):
    ...
    while time.time() < limit:
        ...
        with grpc.insecure_channel("127.0.0.1:{}".format(port)) as channel:
            ...
            ref.str = name
            try:
                stub.RemovePoller(ref, timeout=max(1, limit - time.time()))
                break
            except Exception:
                logger.console("gRPC server not ready")
    ...
    while time.time() < limit:
        try:
            with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
                stub = broker_pb2_grpc.BrokerStub(channel)
                return stub.GetTopology(empty_pb2.Empty(), timeout=max(1, limit - time.time()))
        except Exception as e:
...
def ctn_get_broker_log_level(port, log, timeout=TIMEOUT):
    ...
    while time.time() < limit:
        ...
        with grpc.insecure_channel("127.0.0.1:{}".format(port)) as channel:
            ...
            ref.str_arg = log
            try:
                res = stub.GetLogInfo(ref, timeout=max(1, limit - time.time()))
                res = res.level[log]
                return res
                break
            except Exception:
...
def ctn_broker_get_ba(port: int, ba_id: int, output_file: str, timeout=TIMEOUT):
    ...
    while time.time() < limit:
        ...
        with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
            ...
            try:
                res = stub.GetBa(ref, timeout=max(1, limit - time.time()))
                break
            except grpc.RpcError as rpc_error:
                if rpc_error.code() == grpc.StatusCode.INVALID_ARGUMENT:
                    res = rpc_error.details()
...
def ctn_get_host_poller_id(port: int, host_id: int, timeout=TIMEOUT):
    ...
    while time.time() < limit:
        with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
            ...
            ref.idx = host_id
            try:
                res = stub.GetHost(ref, timeout=max(1, limit - time.time()))
...
def ctn_get_hostgroup(port: int, hostgroup_id: int, timeout=TIMEOUT):
    ...
    while time.time() < limit:
        with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
            ...
            try:
                return stub.GetHostGroup(ref, timeout=max(1, limit - time.time()))
...
def ctn_get_servicegroup(port: int, servicegroup_id: int, timeout=TIMEOUT):
    ...
    while time.time() < limit:
        with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
            ...
            try:
                return stub.GetServiceGroup(ref, timeout=max(1, limit - time.time()))
            except grpc.RpcError as e:
                if e.code() == grpc.StatusCode.NOT_FOUND:
                    return None
                logger.console(f"gRPC error: {e}")
...
def ctn_broker_schedule_host_downtime(hostname: str, duration: int,
    ...
    *Example:*
    ...
    with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
        ...
        response = stub.ScheduleDowntime(req, timeout=GRPC_TIMEOUT)
