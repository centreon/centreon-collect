...
def ctn_engine_config_set_key_value_in_cfg(idx: int, desc: str, key: str, value: str, file: str):
    ...
    if file == "hostgroups.cfg":
        ...
        r = re.compile(rf"^\s*id\s+{desc}\s*$")
    elif file == "severities.cfg":
        r = re.compile(rf"^\s*id\s+{desc}\s*$")
...
def ctn_rename_service_group(index: int, old_servicegroup_name: str, new_service_group_name: str):
    ...
    group_name_search = re.compile(
        rf"^\s+servicegroup_name\s+{old_servicegroup_name}$")
    for i in range(len(ll)):
        line = ll[i]
...
def ctn_dump_conf_info_grpc():
    """
    Retrieve engine conf via a gRPC call.

    Returns:
        A dictionary containing the engine conf, if successfully retrieved.
    """
    limit = time.time() + 30
    while time.time() < limit:
        time.sleep(1)
        with grpc.insecure_channel("127.0.0.1:50001") as channel:
            stub = engine_pb2_grpc.EngineStub(channel)
            try:
                conf = stub.DumpCurrentConfig(empty_pb2.Empty())
                conf_dict = MessageToDict(conf, always_print_fields_with_no_presence=True)
                return conf_dict
            except Exception as e:
                logger.console(f"gRPC server not ready {e}")
                error_details = e.details()                 
...
def ctn_engine_config_del_block_in_cfg(idx: int, type: str, key: str, file):
    ...
    if type == "host":
        ...
        pattern = rf"(?<=\n)define servicegroup\s*\{{.*?servicegroup_name\s+{re.escape(key)}\b.*?\}}\n?"
