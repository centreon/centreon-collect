...
""".format(id, ",".join(mbs))
    ...
    @staticmethod
    def create_severities(poller: int, nb: int, cache_dir: str, offset: int):
        if cache_dir == "":
            config_file = f"{CONF_DIR}/config{poller}/severities.cfg"
        else:
            config_file = f"{cache_dir}/severities.cfg"
...
def ctn_create_severities_file(poller: int, nb: int, cache_dir: str = "", offset: int = 1):
    """
    ...
    Args:
        ...
        cache_dir (str, optional): The directory where to store the file or "" to create it as usual. Defaults to "".
        offset (int, optional): Defaults to 1.
    """
    engine.create_severities(poller, nb, cache_dir, offset)
...
def ctn_config_engine_add_cfg_file(poller: int, cfg: str, centengine: str = ""):
    """
    ...
    Args:
        ...
        centengine (str): the centengine.cfg destination file or "" to use the default value.
    """
    if centengine == "":
        path = f"{CONF_DIR}/config{poller}"
        filename = f"{path}/centengine.cfg"
    else:
        filename = centengine
        path = dirname(centengine)
    ...
    for i in range(len(lines)):
        if r.match(lines[i]):
            file = basename(cfg)
            lines.insert(i, f"cfg_file={path}/{file}\n")
            break
...
def ctn_update_engine_config_paths(dirname: str):
    """ Update the paths declared in the centengine.cfg file to match dirname.

    Args:
        dirname: The path to use.
    """
    filename = f"{dirname}/centengine.cfg"
    with open(filename, "r") as f:
        content = f.readlines()
    for i, line in enumerate(content):
        if line.startswith("cfg_file="):
            ...
            fname = line.split('/')[-1]
            content[i] = f"cfg_file={dirname}/{fname}"
            logger.console(f"line = {content[i]}")
        elif line.startswith("resource_file="):
            # We get just the filename
            fname = line.split('/')[-1]
            content[i] = f"resource_file={dirname}/{fname}"

    with open(filename, "w") as f:
        f.writelines(content)


def ctn_engine_get_current_config(port: int):
    """
    Dump the current Engine Configuration of the poller.

    Args:
        port: the port to access to the gRPC API

    Returns:
        A python dictionary of the dump.
    """
    limit = time.time() + 30
    ts = Timestamp()
    ts.GetCurrentTime()
    while time.time() < limit:
        with grpc.insecure_channel(f"127.0.0.1:{port}") as channel:
            stub = engine_pb2_grpc.EngineStub(channel)
            try:
                res = stub.DumpCurrentConfig(empty_pb2.Empty())
                retval = MessageToDict(res)
                for k in retval:
                    logger.console(k)
                return retval
            except:
