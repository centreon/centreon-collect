...
BA_RATIO_NUMBER_BA_SERVICE
	[Documentation]	With bbdo version 3.0.1, a BA of type 'ratio number' with 2 services and one child BA with 1 service
	[Tags]	Broker	engine	bam
	Clear Commands Status
	Clear Retention
	Config Broker	module
...
BEPB_DIMENSION_BA_EVENT
	[Documentation]	bbdo_version 3 use pb_dimension_ba_event creating a BA with its service and For up to 10 iterations, search the log file for an event with specific attributes related to the BA created earlier.
	[Tags]	Broker	Engine	protobuf	bam	bbdo
	Clear Commands Status
	Clear Retention
...
BEPB_DIMENSION_KPI_EVENT
	[Documentation]	bbdo_version 3 use pb_dimension_kpi_event creating a BA with a service and adds a boolean KPI with a value of 100 and checking if the output created in the databese is equal to the expected output.
	[Tags]	Broker	Engine	protobuf	bam	bbdo
	Clear Commands Status
	Clear Retention
