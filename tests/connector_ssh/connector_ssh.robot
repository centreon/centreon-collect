...
Test6Hosts
    ...
    Run    cat ~testconnssh/.ssh/id_rsa.pub ~testconnssh/.ssh/id_ed25519.pub ~root/.ssh/id_rsa.pub > ~testconnssh/.ssh/authorized_keys
    Run    chown testconnssh: ~testconnssh/.ssh/*
    Run    chmod 600 ~testconnssh/.ssh/*
    Ctn Clear Retention
    Ctn Config Broker    module    ${1}
    Ctn Config Engine    ${1}
    Ctn Engine Config Set Value    ${0}    log_level_commands    trace
    ...
    ...    ${0}
    ...    ssh_linux_snmp
    ...    $USER1$/check_by_ssh -H $HOSTADDRESS$ -l $_HOSTUSER$ -a $_HOSTPASSWORD$ -i $_HOSTIDENTITYFILE$ -C "echo -n toto=$HOSTADDRESS$"
    ...    SSH Connector
    ${run_env}    Ctn Run Env
    Ctn Engine Config Set Value In Hosts    ${0}    host_1    _USER    testconnssh
    Ctn Engine Config Replace Value In Hosts    ${0}    host_1    check_command    ssh_linux_snmp
    Ctn Engine Config Set Value In Hosts    ${0}    host_1    _IDENTITYFILE    /home/testconnssh/.ssh/id_ed25519
    ...
    FOR    ${idx}    IN RANGE    2    7
        ...
        Ctn Engine Config Set Value In Hosts    ${0}    host_${idx}    _IDENTITYFILE    /home/testconnssh/.ssh/id_ed25519
...
#     Ctn Clear Retention
#     Remove File    /etc/centreon-engine-whitelist/test2
#     Ctn Config Broker    module    ${1}
#     Ctn Config Engine    ${1}
#     Ctn Engine Config Set Value    ${0}    log_level_commands    trace
#     Ctn Engine Config Add Command
...
Ctn Prepare ssh
    ...
    Run    su testconnssh -c "ssh-keygen -q -t rsa:2048 -N '' -f ~testconnssh/.ssh/id_rsa"
    Run    su testconnssh -c "ssh-keygen -q -t ed25519 -N '' -f ~testconnssh/.ssh/id_ed25519"
    Run    ssh-keygen -q -t rsa:2048 -N '' -f ~/.ssh/id_rsa
    Run    update-crypto-policies --set DEFAULT:SHA1
    Append To File    /etc/security/access.conf    +:testconnssh:ALL
