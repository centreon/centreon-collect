...
* [Reading Engine configuration](#reading-engine-configuration)
  ...
  * [Cross-poller validation: `load_foreign_objects()`](#cross-poller-validation-load_foreign_objects)
    * [`self`, and the case it covers](#self-and-the-case-it-covers)
    * [When the index is built](#when-the-index-is-built)
    * [Why the index owns its names](#why-the-index-owns-its-names)
    * [What the index changes -- and what it does not](#what-the-index-changes----and-what-it-does-not)
    * [The two validation paths, now aligned](#the-two-validation-paths-now-aligned)
...
## Cross-poller validation: `load_foreign_objects()`

A poller on its own cannot judge everything. If it references a host it does not
define -- a dependency, an escalation, a group -- it has only one answer:
*undefined*. `Broker` knows the configurations of **every** poller, and can
therefore tell an object that truly does not exist from one that **lives
elsewhere**.

That is what `load_foreign_objects()` is for: building, from every stored
`<N>.prot`, the two name indexes then handed to `resolve()`:

```
hosts    : host_name                   → the poller defining it
services : {host_name, description}    → the poller defining it
```

### `self`, and the case it covers

The index is built over **all** the stored configurations, without knowing which
validation will use it: the current poller's own is in there and has to be
filtered out. Hence `foreign_objects::self`, set on every iteration.

This is not mere tidiness. The subtle case is an object **present in the poller's
previously stored configuration and dropped from the one being validated**:
without the filter it would read as "defined on this poller" just as it
disappears from it, and a dangling reference would go unnoticed. With the filter
it reads as undefined, which is the truth.

### When the index is built

```mermaid
sequenceDiagram
    participant W as _post_individual_work<br/>or _post_batch_work
    ...
    W ->> C: a lot of pollers
    loop for each poller of the lot
        alt first poller actually validated
            C ->> F: build the index
            F ->> D: read every <N>.prot
            D -->> F: hosts and services of each poller
            F -->> C: index (names owned)
            Note right of F: Once per cycle.<br/>Deferred: a cycle woken up for nothing pays nothing.
        end
        C ->> C: foreign.objects.self = poller
        C ->> R: read + validate
        R ->> R: parse, expand,<br/>resolve(err, logger, foreign)
    end
    Note right of C: The index is destroyed at the end of the cycle.<br/>It is declared outside the loop:<br/>it must outlive every resolve of the lot.
```

Two properties not to break: the index is built **once per cycle** -- on a
deploy-all, rebuilding it per poller would read the store as many times as there
are pollers -- and it is declared **outside** the loop, because `resolve()`
borrows the names it holds.

### Why the index owns its names

`foreign_objects` is made of `std::string_view`: someone has to own the strings.
That used to be the `State` messages themselves, kept alive for the whole
validation -- tens of megabytes of deserialized configuration held in order to
read a few hundred thousand names out of it.

`foreign_states::names` owns them now (a `std::deque<std::string>`, whose elements
never move -- neither when it grows, nor when the structure is moved), names are
**interned** (a `host_name` shared by its twenty-five services is stored once),
and each message is released as soon as it has been walked. Measured on 10
pollers of 50,000 services (51.6 MB of `.prot`):

| | time (release included) | peak memory |
|---|---|---|
| index borrowing from the messages | 391 ms | 391 MB |
| owning index, interned names | **295 ms** | **82 MB** |

The cost is linear, about 5 ms per megabyte of `.prot`. The remaining 82 MB are
the index itself (500,000 entries); the distinct names now weigh about one
megabyte.

> ⚠️ The time **goes down** although slightly more work is done (the interning),
> and that is no paradox: releasing each message at once lets the allocator reuse
> its blocks for the next poller, instead of growing the heap to 391 MB before
> giving it all back. Measuring this requires timing the **release** too: the
> version that carries its garbage out in its return value makes the caller pay
> for it, after the clock has been stopped.

The bench is `broker/core/config/applier/test/broker_state_foreign_bench.cc`
(`DISABLED_`, so it costs nothing in CI):

```bash
tests/ut_broker --gtest_also_run_disabled_tests --gtest_filter='*ForeignObjectsCost*'
```

### What the index changes -- and what it does not

**It never makes a configuration valid.** All four places that consult it
increment `err.config_errors` **before** asking:
...
An object living on another poller is therefore an error, exactly like one that
does not exist -- a dependency or an escalation does not cross a poller
boundary. What the index buys is the **precision of the diagnostic**:

| Without the index | With the index |
|---|---|
| "Dependent host 'X' … is not defined anywhere!" | "Dependent host 'X' … belongs to poller 2: \<reason\>" |

The difference is not cosmetic for whoever reads the message: the first sends
them hunting for a typo, the second says what actually happened.

### The two validation paths, now aligned

`resolve()` takes the foreign objects as an **empty default parameter**
(`common/engine_conf/state_helper.hh:47`). `CheckPollerConfig` did not pass them,
and so gave the degraded diagnostic -- the verdict itself was the same. Both
paths now build the same index:
...
That leaves `self`, which the request does not carry: it names a directory and
nothing else. Since a poller configuration lives in a directory named after its
id -- which is where the ingestion takes it from, nothing carries it inside the
`.cfg` files -- the endpoint reads it from the directory name. When that name is
not an id, nothing can be excluded: the endpoint then **says so** in a `WARNING`
diagnostic, rather than silently letting through a reference to an object this
very directory has just dropped.
...
- `notification_mode = broker`: Broker becomes the sole authority. PHP sends downtimes,
  ...
  them directly in its database. Broker makes the notification **decision** (contact selection,
  escalations) and dispatches only the **execution** (macro expansion + command launch) to the
  supervising poller through the `pb_notification_execute` event; Engine no longer decides
  notifications on its own, it merely executes what Broker dispatches to it. Engine is never
  informed of downtimes, acknowledgements, or escalation rules. `hostescalation` and
  `serviceescalation` objects are removed from `centengine.cfg` entirely.
...
In `notification_mode = broker`, the notification **decision** (model C, see
`doc/ha-target-architecture`) runs in Broker: the `common/notifications` library
(`notification_manager`) is instantiated there with the
`broker_notification_callbacks` backend, and Broker dispatches the **execution**
(macro expansion + command launch) to the supervising poller through the
`pb_notification_execute` event (`io::bbdo`). The poller only executes it.

### Backend initialization

The backend is injected **at the same place as the timeperiods initialization**
on Broker, and only when `notification_mode = broker`. In engine mode nothing is
instantiated.

### Trigger: a third treatment in `multiplexing::engine`
...
- **Ordering** relative to the cache / `unified_sql` does not matter: in
  ...
  consumer takes the triggering state from the event itself.

### Turning off the decision on Engine

In broker mode, Engine's notification decision must be neutralized to avoid a
**double notification** (a local one from Engine and the one dispatched by
Broker). The mechanism: **at negotiation**, Broker advertises in its `Welcome`
message the `broker_handles_notifications` flag (true when
`notification_mode=broker`); Engine receives it (`cbmod_stream`, `pb_welcome`
case), stores it in `cbmod_state`, and `notifier::notify()` then becomes a
**no-op**. Engine no longer decides; it only **executes** the notifications
Broker dispatches (`execute_broker_notification`, triggered by
`pb_notification_execute`) — that execution path is **independent** of this gate.

Design notes:

- **Sticky flag**: once received, it is not reset if Broker briefly disconnects
  — Engine does not take the decision back, which would reopen the double-
  notification door. A durably absent Broker is an HA concern (out of scope).
- **Startup window**: before negotiation the flag is false, so Engine would
  decide — but it has not run any check yet at that point, so it is harmless.
...
| **T6** | Poller-side execution of dispatched notifications (`pb_notification_execute`) + turning off Engine's decision |
| **T7** | Notification decision (`notification_manager`) in Broker |
...
gantt
    ...
    section Engine
    T6 · pb_notification_execute (poller)  :t6, 2024-01-01, 4d
...
downtimes and acknowledgements — must wait behind hours of accumulated check results before
reaching Broker.
...
|------------|---------------------|-------|
| `pb_downtime`, `pb_acknowledgement` | **Always priority (P)** — age is irrelevant; what matters is whether the downtime or acknowledgement is still active | P |
...
`pb_acknowledgement` is placed in queue P and delivered to Broker immediately,
...
- Events reloaded from `_file` on restart are reclassified based on their **event type** — the
  ...
  `pb_downtime` or `pb_acknowledgement` go back into queue P (they
...
evaluates them as part of its own notification decision (`notification_manager`).
...
constraint in the resource distribution algorithm: because the notification decision runs in
...
In `notification_mode = broker`, Broker makes the notification decision (contact selection +
escalations, from its cache) and dispatches only the execution to the poller that supervises the
resource, through a `pb_notification_execute` BBDO event (`io::bbdo`, Broker→poller). The poller
expands the macros and runs each contact's notification command; it no longer decides
notifications on its own. Suppression by downtimes and acknowledgements happens on Broker,
*before* dispatch. See [Driving the notification decision on Broker](#driving-the-notification-decision-on-broker)
...
Broker makes the whole notification **decision** for the zone (model C). Broker's
`notification_manager` observes the state changes (`pb_host_status`/`pb_service_status`), checks
active downtimes and acknowledgements, **evaluates the resource's `notification_period`** (hence
the [T0: Broker-manageable timeperiods](#prerequisite-timeperiods-must-be-broker-manageable)
prerequisite), selects the contacts and applies the escalation rules — all from Broker's cache
and database. It then dispatches only the **execution** (macro expansion + command launch) to
the poller that supervises the resource, through a `pb_notification_execute` BBDO event:

```protobuf
// io::bbdo — Broker → poller. Carries an already-decided notification: the
// contacts are the exact set Broker selected; the poller only expands macros
// and runs each contact's notification command.
message NotificationExecute {
  uint64 host_id             = 1;
  uint64 service_id          = 2;   // 0 for a host notification
  uint32 category            = 3;
...
On the poller, `execute_broker_notification` resolves the contact names, expands the macros and
runs the notification commands. Engine no longer decides notifications on its own; suppression by
downtimes and acknowledgements happens on Broker, *before* dispatch. See
[Driving the notification decision on Broker](#driving-the-notification-decision-on-broker) for
the trigger mechanism (the third treatment in `multiplexing::engine`).

Advantages:
- The notification decision is centralised (consistent history, suppression and escalations),
  which is what Poller HA needs — a decision about a *logical* resource belongs to the centre.
- No command executor or macro engine has to be reimplemented in Broker: execution stays on the
