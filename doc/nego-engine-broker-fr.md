...
* [Remote Servers et configuration centralisée](#remote-servers-et-configuration-centralisée)
  ...
  * [Évolution de broker\_state](#évolution-de-broker_state)
    * [Connu, connecté, démarré : trois états, trois questions](#connu-connecté-démarré--trois-états-trois-questions)
    * [Un poller derrière un relais se comporte comme un poller direct](#un-poller-derrière-un-relais-se-comporte-comme-un-poller-direct)
...
* [BAM dans Broker](#bam-dans-broker)
  * [Les objets](#les-objets)
  * [Deux streams, un module](#deux-streams-un-module)
    * [Le monitoring stream, événement par événement](#le-monitoring-stream-événement-par-événement)
...
1.  il demande à `inotify` s’il y a eu des modifications dans le
    ...
    Le descripteur étant non bloquant, une file d’événements vide se
    traduit par `EAGAIN` : c’est le cas ordinaire — la plupart des ticks
    ne trouvent rien — et il n’est donc pas journalisé comme une erreur,
    faute de quoi chaque cycle laisserait une ligne `error` derrière lui.
...
`*.lck` dans ce répertoire.

La surveillance est **événementielle** : une attente asynchrone
(`async_wait_readable`) est posée sur le descripteur `inotify`, et rien ne
s’exécute tant que le noyau n’a rien à dire. À son réveil, `Broker` lit les
événements, **réarme aussitôt l’attente** — un lot plus gros qu’un tampon laisse
le descripteur lisible, l’attente suivante aboutit immédiatement et la file est
donc vidée au lieu d’être étalée — puis arme un délai de regroupement.

Ce regroupement de 500 ms est ce qui remplace l’ancien cadencement : chaque
nouvel événement repousse l’échéance, si bien qu’un déploiement de masse, qui
touche un `.lck` par poller en rafale, est traité **en un seul lot** et ne relit
donc qu’une fois les configurations stockées. Un plafond de 5 secondes empêche
un flux continu d’événements de repousser le traitement indéfiniment.

Une fois le lot constitué, `Broker` fait plusieurs tâches :
...
sequenceDiagram
    ...
    W -->> T: réveil : lecture des événements,<br/>attente réarmée aussitôt
    Note right of T: Regroupement de 500 ms (plafond 5 s) :<br/>chaque nouvel événement repousse l'échéance,<br/>une rafale devient un seul lot.
    opt Premier cycle, répertoire de cache connu<br/>après le démarrage, timer de sécurité (5 min),<br/>ou événements incomplets<br/>(IN_Q_OVERFLOW, watch rétabli)
    ...
    alt Configuration invalide
        ...
        T ->> C: supprime X.lck
        Note right of T: L'annonce a été lue et ce qu'elle annonçait<br/>est sur le disque : c'est new-X.prot qui dit<br/>désormais qu'une livraison attend.
        ...
        alt Poller X connecté
            ...
            Note right of T: Rien ne peut lui être livré, et rien n'est<br/>réinjecté : new-X.prot attend sur le disque.
            E ->> S: connexion et négociation BBDO
            S ->> T: add_peer : aucune annonce,<br/>mais new-X.prot est là
            Note right of T: Livraison directe depuis l'état préparé,<br/>sans relire les sources.
        end
    end
```
...
`inotify` qui détecte. Il ne tourne donc pas à chaque tick, mais dans quatre
situations :

1. **Au premier cycle**, déclenché immédiatement au démarrage de la
   surveillance, ce qui ramasse les `.lck` déjà présents (une configuration
   poussée alors que `Broker` était arrêté, ou juste avant que le watcher ne
   soit en place).
2. **Quand le répertoire de cache devient connu après ce démarrage**, la
   surveillance ayant été lancée par la connexion d’un poller avant que la
   configuration ne soit appliquée : le premier cycle n’avait alors rien à
   scanner (voir [Démarrage de la surveillance](#démarrage-de-la-surveillance)).
3. **Quand le noyau dit avoir perdu des événements.** Si la file d’événements
   ...
   spéculation.
4. **Quand le watch a été perdu et rétabli** (voir ci-dessous) : ce qui s’est
   produit dans le répertoire pendant qu’il n’était pas surveillé nous est
   invisible.

En dehors de ces cas, un timer de sécurité à 5 minutes déclenche un scan pour ce
que rien de ce qui précède ne couvre — un répertoire sur un système de fichiers où
`inotify` ne fonctionne pas, par exemple. Ce timer relit aussi les événements, et
c’est important : c’est le seul chemin qui rappelle `watch()`, donc le seul qui
puisse rétablir un watch dont la reprise avait échoué. Une surveillance purement
événementielle ne pourrait jamais se réveiller pour se réparer.

Scanner à chaque tick coûtait un parcours du répertoire et un `stat` par annonce
encore présente, indéfiniment et pour rien.

Tout ce que le scan retrouve est traité, sans exception : une annonce encore là
est, par construction, une poussée que `Broker` n'a pas lue — il ne la laisse
jamais survivre au cycle qui la lit (voir
[Cycle de vie de l'annonce](#cycle-de-vie-de-lannonce)).

> Ce n'était pas le cas tant qu'un `.lck` était conservé jusqu'à la connexion de son
> poller. Le scan retrouvait alors des annonces déjà traitées, et **chaque
...
> avant de conclure qu'il n'y avait rien à faire : un seul poller éteint suffisait à
> faire reparser en permanence toute la configuration de la plateforme. Il a fallu
> un filtre dédié pour l'éviter ; unifier la durée de vie des annonces a supprimé la
> cause, et le filtre avec elle.

#### Les rouages

Quatre choses seulement déclenchent un cycle, et tout le reste en découle : un
événement `inotify`, l'échéance du regroupement, celle du filet de sécurité, et la
connexion d'un poller. Elles se répartissent sur **deux strands** — le premier
surveille et ne travaille jamais, le second lit les configurations — ce qui est ce
qui permet à la file `inotify` de continuer à se vider pendant qu'une configuration
de 50 000 services est lue.

```mermaid
flowchart TD
    PHP["php touche X.lck<br/>ou pollers.lck"]
...
* **La connexion d'un poller doit se réveiller elle-même.** Rien n'a changé sur le
  ...
  poller attendrait le filet de sécurité.

L'état partagé se réduit à trois choses : `_lck_set`, les pollers à traiter, protégé
par un mutex puisqu'il est rempli depuis les deux strands ;
`_scan_requested_by_watcher`, un atomique écrit côté surveillance et lu côté travail ;
et `_burst_started_at`, qui n'appartient qu'à `_watch_strand` et n'a donc besoin
d'aucune protection.
...
#### Deux façons d'annoncer une poussée

PHP dispose de deux formes, et `Broker` lit les deux.

La forme historique est un fichier `<poller_id>.lck` par poller. Son défaut est
qu'elle ne dit pas où s'arrête un export : `Broker` ne peut que regrouper ce qui
arrive de façon rapprochée, alors que l'intervalle entre deux `.lck` est le temps de
génération de la configuration suivante, soit plusieurs secondes sur un gros poller.
Un export portant sur plusieurs pollers se scinde alors en plusieurs lots.

La forme cible est un fichier unique, `pollers.lck`, qui **énumère les pollers du
lot**, un identifiant par ligne. `Broker` n'a plus rien à deviner : il traite
l'export en une passe, quelle que soit la durée qu'a prise sa génération. C'est ce
qui rend correct le déplacement d'un objet d'un poller vers un autre — le diff
global réconcilie alors le retrait et l'ajout en une simple modification, au lieu de
les voir dans deux lots successifs. Le contrat détaillé est décrit dans
[php-evolutions](./php-evolutions-fr.md#évolution-3--un-lot-de-configuration-explicite--pollerslck).

Les deux formes ont la **même durée de vie** : elles annoncent, rien de plus, et
`Broker` les supprime une fois les configurations qu'elles nomment préparées. Ce qui
marque une livraison en attente est le `new-<ID>.prot` que `Broker` écrit dans son
propre répertoire — voir
[Cycle de vie de l'annonce](#cycle-de-vie-de-lannonce). Un `pollers.lck` ne pourrait
d'ailleurs pas tenir ce rôle : il nomme plusieurs pollers, dont les livraisons
n'aboutissent pas ensemble.

La seule différence de traitement qui subsiste est le **regroupement**. Un
...
Un `<ID>.lck` est vide : seul son nom compte, donc n'importe quel événement le
concernant suffit. `pollers.lck` a un **contenu**, et `IN_CREATE` arrive alors que le
fichier est encore vide — y réagir reviendrait à lire un lot incomplet, ou aucun.
Deux événements seulement disent qu'un fichier est fini, et `Broker` n'écoute que
ceux-là pour le fichier de lot :

* `IN_CLOSE_WRITE`, quand celui qui l'écrivait l'a refermé — c'est ce qui rend
  l'écriture directe sûre, et donc permise par le contrat ;
* `IN_MOVED_TO`, quand il est arrivé par renommage.

Les deux doivent être **demandés** dans le masque, comme `IN_DELETE_SELF` et
`IN_MOVE_SELF`. Sans `IN_MOVED_TO`, le `rename()` que le contrat autorise resterait
invisible : le fichier n'apparaîtrait qu'au scan suivant.

##### La livraison à la connexion d'un poller

Quand un poller absent lors du lot démarre, sa configuration est peut-être déjà prête.
`add_peer()` regarde ce qui l'attend, dans cet ordre :

* **une annonce (`<ID>.lck`)** : elle n'a pas encore été lue, donc les sources n'ont
  pas été validées. Le poller retourne dans la liste à traiter pour un **cycle
  complet**, et le *debounce* est réamorcé depuis là — rien n'a bougé dans le
  répertoire, donc `inotify` n'aurait jamais réveillé le veilleur pour ce poller.
* **sinon un `new-<ID>.prot`** : l'état est sur le disque et il ne reste qu'à calculer
  la différence avec ce que le poller annonce faire tourner.
  `_prepare_diff_from_new_prot_file()` s'en charge **directement**, sans repasser par
  un cycle : si `<ID>.prot` existe et correspond à la version annoncée par le poller,
  la différence est calculée ; sinon la configuration complète est envoyée. Relire les
  sources aurait refait la moitié coûteuse d'un cycle — `hash_directory`, `parse`,
  `expand`, `resolve`, et le rechargement de toutes les configurations stockées
  derrière — pour aboutir à un résultat déjà présent sur le disque.
* **ni l'un ni l'autre** : Broker suit le chemin normal — configuration inconnue, et
  c'est au poller de dire ce qu'il a.

**L'ordre découle du passage de témoin, il n'a pas à être justifié par des dates.**
Un cycle supprime l'annonce aussitôt qu'il a écrit le fichier préparé ; une annonce
encore présente porte donc nécessairement sur une poussée **postérieure** au fichier
préparé posé à côté d'elle. Livrer le préparé serait livrer le plus ancien des deux.

C'est ce qui a permis de retirer la comparaison des dates d'écriture des deux
fichiers, ainsi que le filtre qui empêchait le scan de réinjecter un poller éteint :
une annonce encore là est du travail à faire, sans autre question à se poser.

> **Course corrigée.** Ce mécanisme corrige une course où, lorsque tous les pollers
> connus ne sont pas connectés simultanément, Broker publiait le diff global avec un
> sous-ensemble des pollers — ceux dont la configuration avait été envoyée et
> acquittée — et abandonnait celle du poller retardataire. Le `new-<ID>.prot` qui
> l'attend est ce qui lui vaut d'être servi dès sa connexion, et un nouveau diff
> global est alors publié pour l'intégrer en base.

##### Ce que le cycle a fait

Chaque cycle qui a travaillé se termine par une ligne de bilan, parce qu'une
configuration qui ne part nulle part doit se voir sans relecture croisée du journal :

```
Configuration cycle: 3 ready, 1 sent, 0 refused, 2 poller(s) not connected: 2, 3
```

`Broker` ne sait pas combien de pollers compte la plateforme — seuls les pollers
connectés lui sont connus — mais il sait combien de configurations il vient de
préparer, et lesquelles il n'a pu remettre à personne. Les pollers nommés là ont leur
configuration prête sur disque et seront servis dès qu'ils se connecteront.

#### Démarrage de la surveillance

La surveillance a **deux déclencheurs**, et c’est le premier arrivé qui la met en
route : `set_cache_config_dir()`, lorsque la configuration de `Broker` est
appliquée, et `add_peer()`, lorsqu’un poller se connecte. L’ordre n’est pas
garanti — un poller déjà lancé se connecte parfois quelques dizaines de
millisecondes avant que la configuration ne soit appliquée.

Quand c’est `add_peer()` qui gagne, la surveillance démarre **sans savoir où est
le répertoire de cache**, ce qui a trois conséquences immédiates :

1. l’attente `inotify` ne peut pas être armée, faute de descripteur ;
2. le premier cycle ne scanne rien, `_cache_config_dir` étant encore vide ;
3. `_lck_file_for_poller()` renvoie 0 pour le poller qui vient de se
   connecter — il abandonne sans watcher — donc son `.lck` n’est même pas
   cherché.

`set_cache_config_dir()` doit donc, lorsqu’elle trouve la machinerie déjà
lancée, faire **deux** choses : armer l’attente, et **redemander un cycle avec
scan**. Armer seulement ne suffit pas : `inotify` ne signale que ce qui arrive
*après* l’armement, et le poller qui a déclenché ce démarrage précoce est
justement celui dont la configuration a été manquée. Il resterait sans
configuration jusqu’au timer de sécurité, soit cinq minutes.

> **Pourquoi ce point n’existait pas avant.** Le tick de 5 secondes rescannait le
> répertoire indéfiniment : tout ce qui avait été manqué au démarrage était
> rattrapé au cycle suivant, sans que personne n’ait à savoir qu’il avait été
> manqué. Ce rattrapage implicite disparaît avec le cadencement, et ce que le
> démarrage ne fait pas, plus rien ne le fera. L’armement de l’attente est par
> ailleurs rendu idempotent (`_inotify_wait_armed`) : tentée depuis deux endroits,
> deux attentes sur un même descripteur se déclencheraient toutes les deux pour
> un seul événement.

Un dernier point d'ordre, dans `apply()` celle-fois : le répertoire des
configurations de pollers est renseigné **avant** `set_cache_config_dir()`, jamais
après. Poser le répertoire de cache démarre la surveillance, et le cycle qu'elle
poste écrit `new-<ID>.prot` ; sans répertoire de destination, ce chemin serait
relatif au répertoire de travail de `cbd`. C'est le même piège que celui décrit plus
haut, mais dans l'autre sens : ici ce n'est pas la surveillance qui démarre trop tôt,
c'est ce dont elle a besoin qui arrive trop tard.

#### Cycle de vie de l'annonce

Deux fichiers interviennent, et chacun ne dit qu'une chose :

| Fichier | Ce qu'il dit | Qui l'écrit | Qui l'efface |
|---|---|---|---|
| `X.lck` ou `pollers.lck` | une configuration est arrivée et Broker ne l'a pas encore lue | PHP | Broker, dès qu'il l'a lue |
| `new-X.prot` | une configuration est prête et attend d'être acquittée par son poller | Broker | le renommage en `X.prot` à l'acquittement |

L'annonce ne parle donc **pas** de livraison, et sa suppression n'est plus
conditionnée à la présence du poller. Ce que dit `new-X.prot`, il le dit que le
poller soit connecté ou non, et il le dit dans le répertoire que Broker possède
— ce qui est ce qui permet à `pollers.lck` d'exister, puisqu'un fichier de lot
ne peut pas suivre la livraison de chacun des pollers qu'il nomme.

**Les deux marqueurs se passent le témoin.** L'annonce est supprimée dès que
`new-X.prot` est écrit, et **jamais avant** : à aucun instant il n'y a ni l'un ni
l'autre, si bien qu'un échec d'écriture ou un arrêt brutal entre les deux laisse
toujours de quoi retenter. Si l'écriture échoue, l'annonce est conservée
volontairement et le cycle suivant relit les sources.

Ni la détection `inotify` ni le scan de secours ne suppriment l'annonce : lire un
événement n'est pas traiter ce qu'il annonce. Seul le cycle le fait, une fois le
fichier préparé sur le disque.

**Une exception :** si la configuration poussée est **rejetée comme invalide**
(elle échoue à la validation décrite dans
[Valider une configuration de poller](#valider-une-configuration-de-poller--lendpoint-grpc-checkpollerconfig) :
`parse`+`expand`+`resolve` rapportent au moins une erreur), aucun `new-X.prot`
n'est écrit, et l'annonce est pourtant consommée — le seul cas où le témoin n'est
pas passé, faute de successeur. Une configuration structurellement invalide ne
redeviendra jamais valide d'elle-même, et la retenter indéfiniment reviendrait à
...
La disparition de l'annonce signifiait « ta configuration a atteint son poller ».
Elle signifie maintenant « Broker a pris ta configuration en charge », ce qui
est tout ce dont PHP a besoin pour pousser la suivante — et tout ce qu'il peut
utilement attendre, un poller éteint ne pouvant rien acquitter. C'était déjà la
sémantique de `pollers.lck` ; les deux formes disent désormais la même chose.

```mermaid
stateDiagram-v2
    Annoncee: Annoncee - annonce presente, pas encore lue
    Validation: Validation - parse + expand + resolve
    Rejetee: Rejetee - conf refusee, annonce consommee, pas de retry
    Preparee: Preparee - new-X.prot ecrit, annonce supprimee
    Acquittee: Acquittee - DiffState acquitte, new-X.prot renomme en X.prot
    [*] --> Annoncee: PHP ecrit le repertoire puis l annonce
    Annoncee --> Validation: evenement inotify ou scan
    Validation --> Rejetee: config_errors detectees
    Validation --> Annoncee: echec d ecriture, annonce conservee
    Validation --> Preparee: conf valide
    Rejetee --> [*]
    Preparee --> Acquittee: poller connecte, ou des qu il se connecte
    Acquittee --> [*]
```
...
#### Quand un tour de configuration est-il terminé ?

La question n'est pas rhétorique : clore un tour déclenche la construction de la
différence globale, laquelle **consomme et supprime tous les `diff-<ID>.prot`** du
répertoire. Le clore alors qu'un poller a sa différence préparée mais pas encore
remise détruit ce fichier — et il n'est jamais réécrit. Le poller continue alors de
réclamer sa mise à jour et `Broker` échoue indéfiniment à ouvrir un fichier disparu.

`try_close_conf_round()` compte donc, parmi les pairs connectés :

* ceux à qui la configuration a été **envoyée**, et parmi eux ceux qui ont acquitté ;
* ceux dont une configuration est **prête mais pas encore envoyée** — le tour n'est
  pas terminé tant qu'il en reste un.

Un pair connu mais non connecté — un indice restauré du `topology.cache` — n'entre
jamais dans ce décompte, parce que rien n'est préparé pour lui en amont (voir
[Un poller derrière un relais se comporte comme un poller direct](#un-poller-derrière-un-relais-se-comporte-comme-un-poller-direct)).
...
> **Le piège.** Le critère « ce pair a une configuration disponible non envoyée » ne
> peut pas s'écrire `available_conf` non vide, car ce champ n'est **jamais vidé** :
> tous les pairs paraîtraient en attente pour toujours et aucune différence globale
> ne serait plus jamais publiée. Ce qui distingue « une livraison est due » de « la
> dernière a été acquittée », c'est `available_conf != engine_conf`, `engine_conf`
> prenant la version acquittée au moment de l'acquittement. C'est exactement le test
> que fait `poller_needs_update()`, et les deux le partagent désormais
> (`_peer_needs_update()`) : c'est leur divergence qui avait causé la perte de
> configuration décrite plus haut.

> **Le second piège : réarmer un drapeau pour une version déjà en vol.** Le drapeau
> « pas encore envoyée » est posé par `_prepare_diff_for_poller()`, et il ne l'est
> **que si la version change**. PHP est libre d'annoncer deux fois le même export —
> un `<ID>.lck` à l'écriture de la configuration, un second quelques centaines de
> millisecondes plus tard — et le second cycle prépare alors une version identique
> à celle déjà remise au poller. La remettre à « pas encore envoyée » orpheline son
> acquittement : celui-ci cherche un pair marqué comme servi, n'en trouve aucun
> (`0/0 acknowledged`), et le tour ne se clôt jamais. Aucune différence globale n'est
> publiée et **rien n'arrive en base**, alors que le poller a bien répondu.
>
> Le défaut était latent : avec l'ancien tick de 5 secondes, les deux cycles étaient
> trop éloignés pour se chevaucher, l'acquittement arrivant en une centaine de
> millisecondes. Le *debounce* de 500 ms les rapproche assez pour que le second cycle
> coiffe l'acquittement du premier.

Un poller **non connecté** n'entre dans aucun de ces comptes, et c'est correct :
préparer une différence exige un pair, donc un poller éteint n'a aucun
`diff-<ID>.prot` à perdre. Un `1/1 acknowledged` sur une plateforme dont deux pollers
sont éteints est donc légitime — c'est la ligne de bilan du cycle qui lève
l'ambiguïté en nommant les absents.
...
Historiquement, l'essentiel des warnings/erreurs de verify-config était produit
plus tard, par `applier::state::apply` (la phase de *resolve*), via les méthodes
`check()`/`resolve()` des objets runtime d'Engine (`host`, `service`, `contact`,
`notifier`, …). Ce code est propre à Engine, n'est pas lié dans `cbd`, et l'hôte
du Broker central n'a pas forcément `centengine` installé — `Broker` ne peut donc
pas l'exécuter.

**État : une phase `resolve` existe désormais dans `engine_conf`.** Un nouveau
`state_helper::resolve(error_cnt&, logger)` s'exécute **après `expand`** et valide
directement le protobuf `State` (sans objet runtime). Il est **en accumulation
seule** (il ne lève jamais d'exception ; il incrémente seulement
`config_warnings`/`config_errors` et logue), ce qui est exactement ce qu'attend un
validateur « tout rapporter, puis décider ». Il est câblé dans les **trois
consommateurs**, de sorte qu'un check porté a un foyer unique et tourne
...
- **Engine** — `main.cc` (`--verify-config`, et le chemin de démarrage sur un
  `centengine.cfg` non-fiable) et `events/loop.cc` (reload) appellent tous
  `expand` + `resolve` avant l'applier et rejettent une configuration invalide.

  > Note : le chemin de démarrage **proto-fiable** (`state.prot` poussé par
  > Broker) et le chemin runtime `apply_diff` ne re-jouent **pas**
  > volontairement `resolve` — ces configurations ont déjà été validées par
  > Broker avant d'être poussées (un `.prot` écrit à la main n'est pas une entrée
  > supportée). Voir la décision de conception dans les notes de revue.
...
> **Appliqué à la phase resolve.** Les nouveaux validateurs `resolve` suivent
> dès le départ le modèle en accumulation seule : `contact_helper::resolve` ne
> lève jamais d'exception, il logue et incrémente seulement `error_cnt`. Les
> consommateurs décident ensuite quoi faire des totaux (Engine/ingestion
> rejettent quand `config_errors > 0` ; `CheckPollerConfig` rapporte chaque
> diagnostic). La conversion throw→accumulation décrite ci-dessous concerne
> toujours les anciens sites `parse`/`expand`.
...
> **C’est `inotify` qui mène, plus un timer.** Historiquement le descripteur
> n’était lu qu’en réponse à un tick de 5 secondes : la détection était bornée
> par ce délai, la lecture ne consommait qu’un tampon de 4 Ko par tick (une
> centaine d’événements, le reliquat attendant le tick suivant), et un cycle
> était payé même quand rien ne se passait. L’attente asynchrone a supprimé les
> trois : plus rien ne s’exécute au repos, la détection est immédiate, et le
> réarmement après chaque lecture vide la file. Le regroupement de 500 ms décrit
> plus haut conserve la mutualisation que le cadencement apportait, et le timer
> est devenu un filet de sécurité à 5 minutes.
...
sequenceDiagram
  ...
  Note right of B: Une configuration invalide est refusée :<br/>ni new-X.prot, ni diff, son X.lck est consommé.<br/>Seules les configurations valides poursuivent ci-dessous.
...
Dans ces trois cas, Broker enregistre simplement qu'il connaît de nouveau la
configuration de ce poller — `set_broker_knows_poller_conf(id, true)` — sans écraser
le fichier.
...
> poller qui se connecte *après* que PHP a poussé sa configuration ne tombe pas
> ici : ou bien son annonce n’a pas encore été lue et il repart pour un cycle,
> ou bien son `new-<ID>.prot` l’attend et lui est livré directement (voir
> [Cycle de vie de l’annonce](#cycle-de-vie-de-lannonce)) — dans les deux cas la
> livraison reprend par le flux normal au lieu d’un `DiffState{unknown=true}`.
...
sequenceDiagram
    ...
    B ->> B: add_peer()
    note right of B: Broker ne trouve ni <ID>.prot, ni <ID>.lck,<br/>ni new-<ID>.prot pour ce poller.<br/>broker_knows_poller_conf est mis à false.
    ...
    alt state.prot existe (poller_id != 0)
        ...
        note right of B: Si aucun fichier plus récent (.lck,<br/>new-<ID>.prot ou <ID>.prot) n'existe,<br/>Broker écrit <ID>.prot, note qu'il connaît<br/>de nouveau la conf et alimente le cache.
...
stateDiagram-v2
    ...
    note right of watch_engine_conf: Cette fonction récupère les IDs remontés par inotify<br/>(complétés par le scan de secours du répertoire).<br/>Le fichier ID.lck n'est PAS supprimé ici : lire un<br/>événement n'est pas traiter ce qu'il annonce.<br/>C'est le cycle qui le supprime, une fois new-ID.prot écrit.
...
`broker_state` ne possède plus aucune map de pairs : elles vivent toutes dans
`peer_registry`, avec leur propre mutex. Ce qui reste sous
`broker_state::_connected_peers_m`, ce sont uniquement les files de transfert —
`_pending_config_requests`, `_pending_config_revokes`, `_pending_diff_states`,
`_pending_diff_state_acks` et `_engine_configuration`.
...
L'ancienne structure `peer` unique mélangeait des champs spécifiques aux Engine et des champs génériques. Elle a été remplacée par trois structures distinctes, chacune stockée dans sa propre `flat_hash_map`. Les pairs Engine sont indexés par le seul `poller_id` — un poller n'a qu'un Engine — tandis que les deux autres le sont par `peer_key = tuple<poller_id, poller_name, broker_name>`. Toutes, ainsi que le mutex qui les protège, vivent dans `peer_registry` :

```cpp
struct engine_peer {
    uint64_t    poller_id;
    std::string poller_name;
    /* Quand le pair s'est connecté, directement ou via un relais. Vide tant
     * qu'il n'a jamais été vu connecté dans cette session Broker — une entrée
     * restaurée du topology.cache, indice de routage et non pair. */
    ...
    /* Si Broker détient le *contenu* de la version ci-dessus : un <ID>.prot sur
     * disque, ou un <ID>.lck qui en annonce un. Propriété de Broker, pas du
     * poller — les deux sont couramment vraies ensemble. */
    bool        broker_knows_poller_conf;
    uint64_t    via_remote;          // relais devant ce pair, ou 0
    /* Ce que le dernier pb_instance a dit de l'Engine de ce poller. Écrit et lu
...
absl::flat_hash_map<peer_key, unknown_peer> _unknown_peers
    ABSL_GUARDED_BY(_connected_peers_m);
/* Quel relais mène à quel poller, persisté dans topology.cache. */
absl::flat_hash_map<uint64_t, uint64_t>     _poller_to_relay
    ABSL_GUARDED_BY(_connected_peers_m);
```

Trois maps séparées par type éliminent tout dispatch à l'itération : les méthodes qui opèrent sur les engine peers (`poller_needs_update()`, `try_close_conf_round()`, etc.) itèrent directement `_engine_peers` sans test de type. `connected_peers()` agrège les trois maps en un vecteur de `peer` avec le `peer_type` correct pour chaque entrée, en laissant de côté les entrées Engine connues mais non connectées.

Une séquence lecture-modification-écriture sur un pair — lire la version qu'il tourne, construire le diff, armer la nouvelle — ne doit pas s'entrelacer avec une autre sur le même poller, sinon le même fichier serait écrit deux fois. `lock_engine_peer()` rend un handle qui tient le verrou du registre de la première lecture à la dernière écriture ; tous les autres accès passent par les accesseurs simples, qui verrouillent le temps d'une seule opération.
...
### Connu, connecté, démarré : trois états, trois questions

Une entrée de `_engine_peers` ne veut pas dire la même chose selon la façon
dont elle est arrivée là, et les confondre a produit de vrais bugs. Le registre
répond donc à trois questions distinctes.

**Connu** — une entrée existe. Un poller entre dans le registre soit parce
qu'il s'est connecté, soit parce que `topology.cache` disait, au dernier arrêt,
qu'on l'atteignait via un relais. Ce second cas est un *indice de routage* : il
permet de préparer une configuration poussée par PHP pendant la coupure et de
l'adresser au bon relais avant que celui-ci ne revienne. Ce n'est pas une
connexion, et rien de ce qui rend compte des connexions ne doit le compter.

**Connecté** — `engine_peer::connected_since` porte une date. Un
`std::optional<time_t>`, délibérément : l'absence de valeur *est* le « jamais vu
connecté dans cette session », là où une date à zéro le disait implicitement.
Elle est posée par `add_peer()` à la négociation et par
`register_poller_via_relay()` quand un relais parle pour le poller. Elle n'est
jamais effacée : une déconnexion supprime l'entrée entière.

`is_poller_connected(poller_id)` répond à cette question, et c'est sur elle que
filtrent `connected_pollers()` et `connected_peers()` — `GetPollers` et
`GetPeers` ne remontent donc plus les indices restaurés du `topology.cache`
comme des pollers au nom vide connectés depuis 1970.

**Démarré** — `engine_peer::engine_running` dit ce que le dernier `pb_instance`
a annoncé sur l'Engine du poller. Faux au moment de la connexion : un pair peut
très bien être connecté avec son Engine pas encore démarré.

Il est entretenu par `set_instance_running()`, dont l'unique appelant est le
`monitoring_stream` de BAM, et relu par ce seul module. Sur un
`pb_instance(running=false)`, BAM lit le flag *avant* de le basculer : un poller
qui ne tournait pas rejoue un événement qu'il avait conservé, il ne s'arrête
pas, et l'état de downtime de ses KPI ne doit pas être remis à zéro. C'est le
fait d'être l'unique écrivain qui rend cette lecture-puis-bascule fiable, donc
le flag reste volontairement l'affaire de BAM — il est rangé dans le registre
plutôt que dans le module uniquement pour survivre à un redémarrage du flux BAM.

`is_poller_running(poller_id)` répond à cette question. Ni elle ni
`set_instance_running()` n'apparaissent sur la classe de base `state` : elles
n'auraient aucun sens dans `cbmod`, qui ne supervise aucun poller. Elles
s'arrêtent à `broker_state`, et BAM les atteint en descendant le type du state
— avec un garde sur `peer_type()` plutôt qu'une simple affirmation en
...
> `remove_poller()` d'unified_sql demande « ce poller est-il connecté ? » avant de
> supprimer un poller que la base montre encore comme démarré — son dernier
> garde-fou avant d'effacer ses lignes `instances`, `hosts` et `resources`. Comme
> `running` n'était alimenté que par BAM, un Broker sans le module BAM répondait
> « pas connecté » pour tous les pollers et le garde-fou ne tenait jamais. Il
> appelle désormais `is_poller_connected()`.

### Un poller derrière un relais se comporte comme un poller direct

`_prepare_diff_for_poller()` sort immédiatement pour un poller connu mais non
connecté. Sans ce garde, une entrée restaurée du `topology.cache` était traitée
comme un pair à servir : un `diff-<N>.prot` était écrit et `available_conf`
armé pour un poller que personne ne pouvait joindre. `_peer_needs_update()`
restait alors vrai pour lui, `try_close_conf_round()` ne refermait
jamais le round, et le diff global de **tous les autres pollers du round**
était retenu jusqu'au retour du relais.

Un poller connecté en direct mais absent n'a aucune entrée, et sortait donc
déjà immédiatement à cet endroit, laissant son `new-<ID>.prot` sur disque pour
sa prochaine connexion. Le garde donne exactement ce comportement au poller
derrière un relais.

Le manque symétrique était du côté du rattrapage :
`prepare_relay_config_response()` regardait `diff-`, `new-` et `<ID>.prot`,
mais pas un `<ID>.lck` en attente — une configuration annoncée par PHP qu'aucun
cycle n'avait encore lue. Là où le chemin direct réveille le debounce depuis
`_feed_cache_and_wake_up_resources()`, le chemin relais attendait le filet de
sécurité de cinq minutes. Il fait désormais le même examen et le même réveil.
Il répond toujours avec ce qui est disponible plutôt que de retenir l'état plus
ancien comme le fait le chemin direct : un `ConfigRequest` est une
requête/réponse et ne pas y répondre bloquerait la négociation ; la
configuration plus récente suit via
`pollers_via_relay_needing_update()` dans la foulée du debounce.
...
| `peer_registry` | Porter les trois `flat_hash_map` typées — `_engine_peers`, `_broker_peers`, `_unknown_peers` — et `_poller_to_relay`, derrière leur propre mutex ; `broker_state` lui délègue |
...
### Étape 6 — PHP push via relais (✅ implémentée)
  ...
  **Implémentation** : `broker_state::pollers_via_relay_needing_update(relay_id)` collecte les
  pollers dont `via_remote == relay_id` que `_peer_needs_update()` signale comme encore en attente
  de configuration — le même test que partout ailleurs, délibérément non réécrit ici. Dans `broker_stream::read()`, quand `peer_type() == BROKER &&
...
BAM, le *Business Activity Monitoring*, est un module de Broker, `20-bam.so`, chargé par le
cbd central. Il transforme les états des services en états d'*activités métier* (BA), les
publie comme s'il s'agissait de services d'un hôte virtuel, et tient l'historique que le
reporting lit. Ce chapitre décrit son fonctionnement actuel, après les travaux de 2026-09 :
ce qui tourne où, comment il démarre dans les deux régimes de configuration, comment ses
deux streams se partagent le travail, et ce qu'il advient d'une configuration qui boucle
sur elle-même.

## Les objets

Un BA se calcule à partir de *KPI*. Un KPI est l'une de trois choses : un **service** (c'est
son état hard qui compte), un autre **BA** (un BA de BA), ou une **règle booléenne** (une
expression sur des états de services, `{host svc} {IS} {CRITICAL} {OR} ...`). Chaque BA a un
type qui dit comment ses KPI se combinent : `impact` (un niveau qui part de 100 et que
chaque KPI dégradé abaisse), `worst`, `best`, `ratio_number` et `ratio_percent` (combien de
KPI sont CRITICAL). Chaque BA possède un **service virtuel**, `ba_<id>` sur l'hôte
`_Module_BAM_<poller>`, par lequel son état est vu par le reste de la plateforme comme
celui de n'importe quel service.

Seul l'état **hard** d'un KPI participe au calcul. BAM portait aussi un état soft, calculé à
chaque étape et jamais lu : il a été supprimé en 2026-09. Les champs `*_soft` du message
BBDO `KpiStatus` subsistent, remplis avec les valeurs hard, pour que rien en aval n'ait à
changer.

En mémoire, c'est un graphe d'objets `computable` : `kpi_service`, `kpi_ba`, `kpi_boolexp`
aux feuilles, les opérateurs `bool_*` pour les règles, les `ba_*` au sommet. Un enfant qui
change appelle `notify_parents_of_change`, et chaque parent se recalcule à partir de ce seul
enfant (`update_from`), puis notifie ses propres parents s'il a changé à son tour. Les liens
descendants sont des `shared_ptr` ; les liens remontants des `weak_ptr`, pour qu'un BA et
ses KPI ne se maintiennent pas mutuellement en vie au travers des rechargements.
...
## Deux streams, un module

Le module enregistre deux types de sortie, que la configuration de Broker déclare comme
deux sorties distinctes :

| type de sortie | classe | base | rôle |
|---|---|---|---|
| `bam` | `monitoring_stream` | `centreon` (configuration), plus `centreon_storage` pour ce qu'il lit | temps réel : calculer les BA, écrire leur état courant dans `mod_bam` et `mod_bam_kpi`, publier les services virtuels |
| `bam_bi` | `reporting_stream` | `centreon_storage` | historique : événements `mod_bam_reporting_*`, durées par période de reporting, disponibilités quotidiennes |

Ils ne se parlent pas directement. Tout passe par le moteur de multiplexing, chaque stream
déclarant les types d'événements qu'il veut (`connector.cc`). Le monitoring stream lit ce
que les pollers envoient — statuts de services, acquittements, downtimes, instances — et ce
que BAM produit lui-même : `BaStatus`, `KpiStatus`, `InheritedDowntime`. Le reporting stream
ne lit que les événements BI propres à BAM : `BaEvent`, `KpiEvent`, `BaDurationEvent`, les
événements de dimension, et le signal `rebuild`.

```mermaid
flowchart LR
    P[pollers] -->|statuts de services, acks, downtimes| M[multiplexing]
    M -->|événements neb| MS[monitoring_stream]
    MS -->|"BaStatus, KpiStatus\nBaEvent, KpiEvent\nstatut du service virtuel"| M
    M -->|BaStatus, KpiStatus| MS
    M -->|BaEvent, KpiEvent, dimensions| RS[reporting_stream]
    M -->|statut du service virtuel| U[unified_sql, rrd, ...]
    MS -->|UPDATE mod_bam, mod_bam_kpi| DBC[(centreon)]
    RS -->|mod_bam_reporting_*| DBS[(centreon_storage)]
    MS -.->|SCHEDULE_FORCED_SVC_CHECK\nSCHEDULE/DEL_SVC_DOWNTIME| E[fichier de commandes Engine]
```

La boucle à gauche est voulue : un `BaStatus` ou un `KpiStatus` issu d'un calcul est publié
comme n'importe quel événement, et c'est le même monitoring stream qui le reçoit en retour
et l'écrit en base, avec le même groupement (`bulk_or_multi`) que tout le reste. Les
événements BI prennent l'autre branche et finissent dans le reporting stream.

### Le monitoring stream, événement par événement

Un statut de service entre dans le `service_book`, qui sait quels auditeurs suivent chaque
`(host_id, service_id)` : les objets `kpi_service`, et les BA dont c'est le service virtuel.
Le KPI met son état à jour et, **seulement si quelque chose a changé**, se visite — ce qui
produit un `KpiStatus` et, sur un changement d'état, ferme le `KpiEvent` ouvert et en ouvre
un nouveau — puis notifie son BA. Le BA se recalcule, se visite à son tour (`BaStatus`,
`BaEvent`, et le statut de son service virtuel quand son état a bougé), calcule sa downtime
héritée, et notifie ses propres parents. Tous les événements d'un statut entrant sont
collectés par un `event_cache_visitor` et publiés vers le multiplexing **en un seul lot**,
dans l'ordre autres, événements BA, événements KPI.
...
sequenceDiagram
    ...
    participant DB as base centreon
    MX->>MS: write(pb_service_status)
    MS->>SB: update(status, visitor)
    SB->>K: service_update(status, visitor)
    K->>K: changed = état hard ou type d'état modifié
    alt changé, ou événement ouvert désynchronisé
        K->>V: KpiStatus (+ KpiEvent fermé / ouvert)
        K->>B: notify_parents_of_change
        B->>B: update_from(kpi) : _apply_changes, downtime héritée
        B->>V: BaStatus (+ BaEvent, statut du service virtuel)
        B->>B: notify_parents_of_change (kpi_ba au-dessus, s'il y en a)
    else rien n'a changé
        Note over K: aucun événement : l'UPDATE aurait été identique
    end
    MS->>V: commit_to(publisher)
    V->>MX: publish(lot)
    MX->>MS: write(BaStatus), write(KpiStatus)
    MS->>DB: UPDATE mod_bam / mod_bam_kpi (bulk_or_multi)
    MX-->>MS: (BaEvent, KpiEvent partent vers reporting_stream)
```

Le « seulement si quelque chose a changé » est récent (2026-09). Un `KpiStatus` était
produit pour chaque résultat de check de chaque service KPI, changé ou non, et chacun
devenait un `UPDATE mod_bam_kpi` identique au précédent. La visite reste due quand le KPI a
changé, quand il a des événements restaurés de la base en attente d'être commités, ou quand
son événement ouvert ne correspond plus à son état — le cas d'un état restauré depuis le
cache, qui met à jour sans visiter.

Deux voies latérales sortent du stream :

* quand un BA change d'état, un `SCHEDULE_FORCED_SVC_CHECK` de son service virtuel est mis
  en file pour Engine, groupé et envoyé par le fichier de commandes quelques secondes plus
  tard ;
* la **downtime héritée** : un BA configuré pour hériter des downtimes passe en downtime
  quand tous ses KPI impactants le sont, et en sort quand l'un d'eux ne l'est plus. Dans le
  régime où Engine gère les downtimes, c'est un `SCHEDULE_SVC_DOWNTIME` /
  `DEL_SVC_DOWNTIME_FULL` sur le service virtuel, envoyé par le fichier de commandes ; quand
  Broker possède les downtimes (`notification_mode=broker`), le `downtime_manager` interne
  est piloté directement.

### Le reporting stream

Il reçoit les événements BI et les écrit : un `BaEvent` ou un `KpiEvent` est inséré la
première fois qu'il est vu et mis à jour quand il se ferme ; un `BaEvent` fermé est
découpé en une durée par période de reporting du BA, grâce aux objets `timeperiod` de la
bibliothèque partagée, et écrit dans `mod_bam_reporting_ba_events_durations`. Les
événements de **dimension** décrivent la configuration (BA, BV, KPI, périodes, relations)
...
rétention peuvent encore en livrer, et `write()` les convertit en protobuf à l'entrée avec
`neb::bbdo2_to_bbdo3` avant tout traitement. Les deux implémentations parallèles qui
existaient pour chaque type d'événement — une par forme — ont disparu.

### Le rebuild

Quand les périodes de reporting d'un BA changent, PHP le marque `must_be_rebuild` dans
`mod_bam`. Le monitoring stream lit le drapeau quand il applique sa configuration, publie un
événement `rebuild` nommant les BA, et efface le drapeau. Le reporting stream supprime
toutes les durées de ces BA, relit leurs événements clos, recalcule les durées et les
écrit, puis demande au thread des disponibilités de recalculer les leurs. Tout cela sous le
verrou du thread des disponibilités.
...
sequenceDiagram
    ...
    MS->>DB: SELECT ... WHERE must_be_rebuild='1'  (à update())
    MS->>MX: rebuild("1, 2, 3")
    MS->>DB: UPDATE mod_bam SET must_be_rebuild='0'
    MX->>RS: write(rebuild)
    RS->>AV: verrou
    RS->>DB: DELETE des durées des BA 1, 2, 3
    RS->>DB: SELECT des événements clos des BA 1, 2, 3, avec leur ba_event_id
    loop chaque événement clos
        RS->>RS: _compute_event_durations(événement, sink) : une durée par période de reporting
        RS->>RS: sink : ajoute la ligne au lot (ba_event_id connu, pas de sous-requête)
        opt toutes les 1000 lignes
            RS->>DB: INSERT bulk + commit  (une seule connexion)
        end
    end
    RS->>DB: commit(conn) : barrière, tout ce qui était en file est exécuté
    RS->>AV: déverrouille, rebuild_availabilities("1, 2, 3")
    AV->>DB: DELETE des disponibilités, recalcul jour par jour depuis les durées
```

Mesuré sur 20 000 durées, ce rebuild prenait 9,9 s : un statement `UPDATE` puis `INSERT` par
durée, chacun dans sa propre transaction puisque les sorties BAM tournent avec
`queries_per_transaction` à 0, c'est-à-dire en autocommit, donc chacun payait un commit. Les
durées sont maintenant insérées par lots de 1000 lignes, un statement chacun, sur une
connexion épinglée, avec un commit final qui sert de barrière : 0,18 s. Le bench
`tests/benchmarks/bam-rebuild` le mesure.

## Le démarrage, dans les deux régimes

Le monitoring stream ne lit pas sa configuration à sa construction. Le failover qui possède
le stream appelle `update()` une fois le stream ouvert, puis à chaque rechargement de Broker
(`SIGHUP`) : `reader_v2` lit les tables BAM de la base de configuration — BA, KPI, règles
booléennes, et l'état dans lequel chaque objet a été vu pour la dernière fois — et
`configuration::applier::state` applique le résultat, en créant, modifiant ou supprimant les
objets en mémoire par différence avec l'application précédente.
...
sequenceDiagram
    ...
    participant C as cache broker / mapping local
    participant MX as multiplexing
    F->>MS: update()  (après open, puis à chaque SIGHUP)
    MS->>R: read(state)
    R->>DB: BA, KPI, règles booléennes, derniers états connus
    alt configuration legacy
        R->>DB: couples host/service nommés par les règles, activation des services KPI
        R->>C: remplit local_hst_svc_mapping
    else configuration centralisée
        Note over R,C: rien à charger : global_hst_svc_mapping répond depuis le cache broker
    end
    MS->>A: apply(state)
    A->>A: _circular_check : graphe, parcours, cycles -> KPI à écarter, BA à invalider
    A->>A: applique BA, règles (exp_parser + exp_builder), KPI moins les cycles
    A->>C: get_service_id / get_activated pour chaque règle et KPI
    A->>A: BA d'un cycle : set_valid(false, "Circular definition detected...")
    opt première application seulement
        MS->>A: restore_inherited_downtimes()
    end
    MS->>DB: SELECT ba_id FROM mod_bam WHERE must_be_rebuild
    MS->>MX: rebuild(bas)  (s'il y en a)
    MS->>MS: _read_cache() : états des services, commandes externes en attente
    MS->>MX: initialize() : visite chaque BA une fois, un seul lot
```

La seule chose qui diffère entre les deux régimes de configuration est **la résolution des
noms en identifiants**. Une configuration BAM nomme un service à un seul endroit, le
`{host service}` d'une règle booléenne ; tout le reste porte déjà des identifiants. Et
l'applier des KPI doit savoir si un service KPI est toujours activé, pour écarter le KPI
sinon.

* En configuration **legacy**, `reader_v2` construit un `local_hst_svc_mapping` depuis les
  tables `host` et `service` : une requête qui ne résout que les couples effectivement
  nommés par les règles, et la colonne d'activation des services visés par les KPI.
* En configuration **centralisée** (`supports_centralized_conf()`), le mapping est un
  `global_hst_svc_mapping` qui répond depuis le **cache global de Broker**, celui qui
  détient les configurations des pollers. Aucune requête n'est faite : un service absent du
  cache est un service qu'aucun poller n'a, et son KPI est écarté exactement comme celui d'un
  service désactivé en legacy. Les deux régimes doivent écarter les mêmes KPI —
  `BA_DEACTIVATED_SERVICE` et `CBABOODEACTIVATEDSVC` le vérifient.

L'état d'exécution des services KPI n'est pas en base ; il est dans un fichier de cache,
`<broker>.cache.centreon-bam-monitoring`, écrit à l'arrêt du stream et relu après
l'application de la configuration : le dernier état connu de chaque service suivi, et les
commandes externes mises en file et pas encore envoyées. Les BA sont ensuite visités une
fois, ce qui publie l'état restauré et cohérent d'un seul coup.

Une chose n'est ni en base ni dans le cache : le fait qu'une downtime de BA soit
**héritée**. `mod_bam` dit seulement que le BA est en downtime. Jusqu'en 2026-09 l'objet de
downtime héritée en mémoire était reconstruit par accident — l'état soft, jamais
initialisé, faisait passer la restauration du cache pour un changement, ce qui faisait
recalculer chaque BA. Le soft disparu, la reconstruction est explicite : à la première
application, un BA qui hérite des downtimes, que la base dit en downtime, et dont tous les
KPI impactants sont en downtime, retrouve son objet, silencieusement, puisque la downtime
...
y compris le service virtuel d'un BA. Rien dans l'interface web n'empêche un BA d'être, en
un nombre quelconque d'étapes, son propre ancêtre. Une telle configuration ne se calcule
pas : chaque changement se propagerait autour du cycle sans fin.

Avant d'appliquer quoi que ce soit, `applier::state` construit un graphe de la
configuration — BA, KPI, règles, services — avec une arête partout où un changement se
propage : de la source d'un KPI (service, BA ou règle) vers le KPI, du KPI vers son BA, d'un
BA vers son service virtuel, d'un service vers les règles qui le nomment. Un parcours en
profondeur avec une pile explicite trouve les cycles ; un nœud revu alors qu'il est encore
sur la pile en ferme un, et le chemin de ce nœud jusqu'au sommet est le cycle.

Ce qu'on en fait a changé en 2026-09. La vérification levait une exception, ce qui arrêtait
toute l'application : pas un BA n'était calculé, et le failover se reconnectait sans fin,
relisant la même configuration et relevant la même exception. Une boucle dans la
configuration désactivait BAM. Désormais :

* chaque KPI d'un cycle est retiré de la configuration avant son application, ce qui casse
  le cycle quel que soit l'ordre de parcours du graphe ;
* chaque BA d'un cycle est appliqué, mais **invalide** : UNKNOWN quoi que son type
  calcule, avec pour sortie `Circular definition detected. BA <nom> includes itself as a
  KPI.`. Cette sortie est ce que le monitoring stream écrit dans la colonne `comment` de
  `mod_bam`, donc l'utilisateur la voit dans l'interface. Chaque type de BA honore le
  drapeau d'invalidité dans son état et dans sa sortie — seul `impact` le faisait pour
  l'état, aucun pour la sortie — ce qui couvre aussi l'autre cause d'invalidité, un KPI dont
  la cible n'existe pas ;
* le cycle est journalisé en clair, `BA 1 -> KPI 11 -> BA 2 -> KPI 10 -> BA 1` ;
* tout ce qui est hors du cycle est appliqué et calculé normalement. L'application
  suivante, une fois le KPI fautif retiré par l'utilisateur, revalide les BA.

`ApplierCircular` (unitaire) et `BAM_CIRCULAR` (robot) le couvrent : une boucle de BA de BA
avec un BA sain à côté, la boucle corrigée au rechargement, et une boucle par règle
booléenne.

## Ce qui a été mesuré, et où

Le répertoire `tests/benchmarks/` contient deux benchs BAM, filés dans le même store que les
autres et comparés avec `./bench.py compare` :

* `bam-startup` : le temps de chaque étape de `reader_v2::read()` quand la plateforme et la
  configuration BAM grandissent, séparément ;
* `bam-rebuild` : le rebuild BI des durées d'événements quand l'historique grandit.

Deux leçons du second valent d'être gardées avant d'optimiser quoi que ce soit qui écrit
dans MariaDB depuis Broker : regarder comment la connexion commite avant de changer comment
elle envoie, et ne pas se fier au seul profil du client quand le client attend. Un bulk de N
lignes commite une fois, en autocommit aussi ; N statements commitent N fois.
