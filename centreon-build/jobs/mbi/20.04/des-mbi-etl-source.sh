...
# Get version.
cd "$PROJECT"
VERSION=`grep Version packaging/$PROJECT.spectemplate | cut -d ' ' -f 9`
...
put_internal_source "mbi-etl" "$PROJECT-$VERSION-$RELEASE" "$PROJECT-$VERSION.tar.gz"
put_internal_source "mbi-etl" "$PROJECT-$VERSION-$RELEASE" "$PROJECT-git.tar.gz"
