...
containerid=`docker create -e "GITHUB_TOKEN=$GITHUB_TOKEN" $UT_IMAGE /usr/local/bin/unittest.sh $PROJECT`

# Copy sources to container.
docker cp `dirname $0`/mon-ppe-unittest.container.sh "$containerid:/usr/local/bin/unittest.sh"
