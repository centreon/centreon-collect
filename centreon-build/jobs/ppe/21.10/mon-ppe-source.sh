...
curl -F "file=@$PROJECT-$VERSION.tar.gz" -F "version=80" 'https://encode.centreon.com/api/index.php' -o "$PROJECT-$VERSION-php73.tar.gz"
put_internal_source "ppe" "$PROJECT-$VERSION-$RELEASE" "$PROJECT-$VERSION.tar.gz"
put_internal_source "ppe" "$PROJECT-$VERSION-$RELEASE" "$PROJECT-$VERSION-php80.tar.gz"
