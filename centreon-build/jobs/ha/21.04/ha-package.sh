...
cp `dirname $0`"/../../../packaging/ha/21.04/$PROJECT.spectemplate" input/
...
put_internal_rpms "21.04" "$DISTRIB" "noarch" "ha" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "standard" "21.04" "$DISTRIB" "noarch" "ha" "$PROJECT-$VERSION-$RELEASE"
fi
