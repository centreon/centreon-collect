...
if [ "$DEV_TYPE" = "backend" ]; then
  ...
  tar xzf node_modules.tar.gz -C "$PROJECT-$VERSION/www/modules/centreon-bam-server/react/"
fi
...
docker cp `dirname $0`/des-bam-unittest-$DEV_TYPE.container.sh "$containerid:/usr/local/bin/unittest.sh"
