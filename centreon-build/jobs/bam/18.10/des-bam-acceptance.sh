...
sed 's#@WEB_IMAGE@#'$BAM_IMAGE'#g' < `dirname $0`/../../../containers/web/18.10/docker-compose.yml.in > "$PROJECT-$VERSION-full/docker-compose-bam.yml"
...
composer install
find features -type f -name "*.feature" | sort | parallel ./vendor/bin/behat --format=pretty --out=std --format=junit --out="../xunit-reports/{/.}" "{}" || true
