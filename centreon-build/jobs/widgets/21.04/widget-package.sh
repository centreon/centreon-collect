...
# Publish RPMs.
put_internal_rpms "21.04" "$DISTRIBCODENAME" "noarch" "widget-$WIDGET" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "standard" "21.04" "$DISTRIBCODENAME" "noarch" "widget-$WIDGET" "$PROJECT-$VERSION-$RELEASE"
fi
