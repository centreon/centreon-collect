...
cp `dirname $0`"/../../../packaging/agent-config/20.04/$PROJECT.spectemplate" input/
...
put_internal_rpms "20.04" "$DISTRIB" "noarch" "agent-config" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "standard" "20.04" "el7" "noarch" "agent-config" "$PROJECT-$VERSION-$RELEASE"
fi
