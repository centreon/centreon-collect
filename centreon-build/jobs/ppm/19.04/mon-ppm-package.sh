...
put_internal_rpms "19.04" "$DISTRIB" "noarch" "ppm" "$PROJECT-$VERSION-$RELEASE" output/noarch/*.rpm
if [ "$BRANCH_NAME" '=' 'master' -o "$BRANCH_NAME" '=' '19.04.x' ] ; then
  copy_internal_rpms_to_canary "standard" "19.04" "el7" "noarch" "ppm" "$PROJECT-$VERSION-$RELEASE"
fi
