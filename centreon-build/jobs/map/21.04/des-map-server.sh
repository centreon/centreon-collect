...
BUILD_IMAGE="registry.centreon.com/map-build-dependencies-21.04:$DISTRIB"
...
put_internal_rpms "21.04" "$DISTRIB" "noarch" "map-server$flavor" "$PROJECT-server-$VERSIONSERVER-$RELEASE" $FILES_MAP_SERVER
SSH_REPO='ssh -o StrictHostKeyChecking=no ubuntu@srvi-repo.int.centreon.com'
$SSH_REPO rpm --resign "/srv/yum/internal/21.04/$DISTRIB/noarch/map-server$flavor/$PROJECT-server-$VERSIONSERVER-$RELEASE/*.rpm"
$SSH_REPO createrepo "/srv/yum/internal/21.04/$DISTRIB/noarch/map-server$flavor/$PROJECT-server-$VERSIONSERVER-$RELEASE"
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "map" "21.04" "$DISTRIB" "noarch" "map-server$flavor" "$PROJECT-server-$VERSIONSERVER-$RELEASE"
fi
