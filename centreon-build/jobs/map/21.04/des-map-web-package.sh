...
# Fetch sources.
cd input
get_internal_source "map-web/$PROJECT-web-$VERSIONWEB-$RELEASE/$PACKAGE-$VERSIONWEB-php73.tar.gz"
...
put_internal_rpms "21.04" "$DISTRIB" "noarch" "map-web" "$PROJECT-web-$VERSIONWEB-$RELEASE" output/noarch/*.rpm
if [ "$BUILD" '=' 'REFERENCE' ] ; then
  copy_internal_rpms_to_canary "map" "21.04" "$DISTRIB" "noarch" "map-web" "$PROJECT-web-$VERSIONWEB-$RELEASE"
fi
