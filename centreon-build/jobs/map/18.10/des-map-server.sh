...
cd ..
FILES_TOMCAT7="$PROJECT-server-$VERSION/com.centreon.studio.server.parent/com.centreon.studio.map.server/com.centreon.studio.map.server.packaging/com.centreon.studio.map.server.packaging.tomcat7/target/rpm/centreon-map-server/RPMS/noarch/"'*.rpm'
put_internal_rpms "18.10" "el7" "noarch" "map-server" "$PROJECT-$VERSION-$RELEASE" $FILES_TOMCAT7
SSH_REPO='ssh -o StrictHostKeyChecking=no ubuntu@srvi-repo.int.centreon.com'
$SSH_REPO rpm --resign "/srv/yum/internal/18.10/el7/noarch/map-server/$PROJECT-$VERSION-$RELEASE/*.rpm"
$SSH_REPO createrepo "/srv/yum/internal/18.10/el7/noarch/map-server/$PROJECT-$VERSION-$RELEASE"
