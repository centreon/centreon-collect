...
if [ "$BUILD" '=' 'RELEASE' ] ; then
  ...
  if [ "$PRODUCT" '=' 'server' -o "$PRODUCT" '=' 'all' ] ; then
    copy_internal_rpms_to_testing "map" "19.10" "el7" "noarch" "map-server" "$PROJECT-server-$VERSIONSERVER-$RELEASE"
  fi
  if [ "$PRODUCT" '=' 'web' -o "$PRODUCT" '=' 'all' ] ; then
    copy_internal_rpms_to_testing "map" "19.10" "el7" "noarch" "map-web" "$PROJECT-web-$VERSIONWEB-$RELEASE"
  fi
