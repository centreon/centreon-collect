...
MEDIAWIKI_IMAGE=ci.int.centreon.com:5000/mon-mediawiki-18.10:latest
...
sed 's#@WEB_IMAGE@#'$WEB_IMAGE'#g' < `dirname $0`/../../../containers/web/18.10/docker-compose.yml.in > "$PROJECT-$VERSION/docker-compose-web.yml"
sed 's#@WEB_IMAGE@#'$WEB_FRESH_IMAGE'#g' < `dirname $0`/../../../containers/web/18.10/docker-compose.yml.in > "$PROJECT-$VERSION/docker-compose-web-fresh.yml"
sed 's#@WEB_IMAGE@#'$WEB_WIDGETS_IMAGE'#g' < `dirname $0`/../../../containers/web/18.10/docker-compose.yml.in > "$PROJECT-$VERSION/docker-compose-web-widgets.yml"
...
sed 's#@WEB_IMAGE@#'$WEB_IMAGE'#g' < `dirname $0`/../../../containers/mediawiki/18.10/docker-compose.yml.in > "$PROJECT-$VERSION/docker-compose-web-kb.yml"
...
sed 's#@WEB_IMAGE@#'$WEB_IMAGE'#g' < `dirname $0`/../../../containers/web/18.10/docker-compose-influxdb.yml.in > "$PROJECT-$VERSION/docker-compose-web-influxdb.yml"
